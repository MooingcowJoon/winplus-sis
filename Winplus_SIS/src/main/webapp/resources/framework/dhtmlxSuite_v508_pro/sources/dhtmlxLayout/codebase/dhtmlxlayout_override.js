...
function dhtmlXLayoutObject(base, pattern, skin) {
	...
	this.divId = base.parent;
	/* #Customized (조승현) : div id 저장 끝*/
	
	/* #Customized (조승현) : 하위레이아웃 맵, 콜백 저장 시작*/
	this.childLayoutObjMap = {}; //key : divId, value : layoutObj
	this.childLayoutObjCallbackMap = {}; //key : divId, value : callBackfunction
	/* #Customized (조승현) : 하위레이아웃 맵, 콜백 저장 끝*/
	
	/* #Customized (조승현) : 하위 그리드, 콜백 저장 시작*/
	this.childGridObjMap = {}; //key : divId, value : gridObj
	this.childGridObjCallbackMap = {}; //key : divId, value : callBackfunction
	/* #Customized (조승현) : 하위 그리드, 콜백 저장 끝*/
	...
	if (base != null && typeof(base) == "object" && typeof(base.tagName) == "undefined" && base._isCell != true) {
		...
		/* #Customized (김종훈) : 풀스크린 옵션 추가 - 시작 */
		if (base.fullScreen != null) autoload.fullScreen = base.fullScreen;
		/* #Customized (김종훈) : 풀스크린 옵션 추가 - 끝 */
		base = base.parent;
	}
	...
	/* #Customized (김종훈) : 풀스크린 옵션 추가 - 백업 시작 */
	/* window.dhtmlXCellTop.apply(this, [base, (autoload==null?null:autoload.offsets)]); */
	/* #Customized (김종훈) : 풀스크린 옵션 추가 - 백업 끝 */
	/* #Customized (김종훈) : 풀스크린 옵션 추가 - 시작 */
	window.dhtmlXCellTop.apply(this, [base, (autoload==null?null:autoload.offsets), (autoload==null?null:autoload.fullScreen)]);
	/* #Customized (김종훈) : 풀스크린 옵션 추가 - 끝 */
	...
	/* #Customized (조승현) : 재귀 함수를 통한 하위 레이아웃 리사이즈 호출 시작 */
	if(this.divId == document.body){
		var childLayoutResize = function(layoutObj){
			for(var key in layoutObj.childLayoutObjMap){
				//console.log("divId : " + key);
				layoutObj.childLayoutObjMap[key].setSizes();
				if(typeof (layoutObj.childLayoutObjCallbackMap[key]) === "function"){
					layoutObj.childLayoutObjCallbackMap[key]();
				}
				childLayoutResize(layoutObj.childLayoutObjMap[key]);
			}
			for(var key in layoutObj.childGridObjMap){
				layoutObj.childGridObjMap[key].setSizes();
				if(typeof (layoutObj.childGridObjCallbackMap[key]) === "function"){
					layoutObj.childGridObjCallbackMap[key]();
				}
			}
		}
		
		var onChangeWindowSize = function(){
			childLayoutResize(this);
		}
		
		this.attachEvent("onPanelResizeFinish", onChangeWindowSize);
		this.attachEvent("onCollapse", onChangeWindowSize);
		this.attachEvent("onExpand", onChangeWindowSize); 
		this.attachEvent("onResizeFinish", onChangeWindowSize);
		
		window["static_total_layout"] = this;
	}
	/* #Customized (조승현) : 재귀 함수를 통한 하위 레이아웃 리사이즈 호출 끝 */
...
/* #Customized (조승현) : 텝 셋사이즈 호출을 위한 텝 등록 시작*/
dhtmlXLayoutObject.prototype.registTab = function(tabBarObj){
	for(var tabId in tabBarObj.t){
		var divId = tabBarObj.t[tabId].cell.cell.children[0].children[0].id;
		if(divId == this.divId){
			if(typeof window["registered_layout"] == "undefined" || window["registered_layout"] == undefined || window["registered_layout"] == null){
				window["registered_layout"] = {};
			}
			window["registered_layout"][tabId] = this;	
		}
	}
}
/* #Customized (조승현) : 텝 셋사이즈 호출을 위한 텝 등록 끝*/


/* #Customized (조승현) : 하위 레이아웃, 콜백 저장 시작*/
dhtmlXLayoutObject.prototype.captureEventOnParentResize = function(parentLayoutObj,callback){
	parentLayoutObj.childLayoutObjMap[this.divId]=this;
	parentLayoutObj.childLayoutObjCallbackMap[this.divId]=callback;
}
/* #Customized (조승현) : 하위 레이아웃, 콜백 저장 끝*/
...
window.dhtmlXLayoutCell = function(id, layout) {
	...
	if (this.conf.skin == "material") {
		...
		/* #Customized (정혜원) : 최소높이값지정 0으로 변경 (기존 26) 시작*/
		this.conf.min_height = 0;
		/* #Customized (정혜원) : 최소높이값지정 0으로 변경 (기존 26) 끝*/
	} else {
