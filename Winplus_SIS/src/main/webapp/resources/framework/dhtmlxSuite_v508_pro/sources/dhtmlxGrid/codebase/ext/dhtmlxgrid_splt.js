...
dhtmlXGridObject.prototype.splitAt=function(ind){
    ...
    /* #Customized (김종훈) : Grid가 Split 되면 div_grid_split Class 부여 - 시작 */
    /* ※ Split되면 규격이 깨지는 문제가 발생하여 CSS로 잡아야 하므로 CSS 적용을 위한 Class 부여 */
    if(this.entBox.className == undefined || this.entBox.className == null || this.entBox.className == ""){
    	this.entBox.className = "div_grid_split";
    } else {
    	this.entBox.className += " div_grid_split";
    }
    /* #Customized (김종훈) : Grid가 Split 되면 div_grid_split Class 부여 - 끝 */
    ...
    this._fake.callEvent=function(){
    	...
    	/* #Customized (김종훈) : Event 내에서 Grid가 삭제되면 오류가 발생하여 예외 처리 소스 추가 -- 시작 */   	
    	if (!this._fake) {
    		return;
    	}
    	/* #Customized (김종훈) : Event 내에서 Grid가 삭제되면 오류가 발생하여 예외 처리 소스 추가 -- 끝 */
...
var quirks = (_isIE && document.compatMode=="BackCompat");
		...
		*/
            ...
            this.setSizes=function(){
                    ...
                    /* #Customized (김종훈) : Width, Height 오류 수정용 소스 추가 - 시작 */
    				this._fake.entBox.style.width=this._fake.hdr.style.width;
    				this._fake.objBox.style.width=this._fake.hdr.style.width;
    				
    				var fullHeight = this.globalBox.parentElement.offsetHeight;
    				var objHeight = fullHeight;
    				    				
    				if(this.entBox.childNodes){
    					for(var i = 0; i < this.entBox.childNodes.length; i++){
    						var child = this.entBox.childNodes[i];
    						if(child != this.objBox){
    							objHeight -= child.offsetHeight;
    						}
    					}    					
    				}    				
    				
    				this.globalBox.style.height = (fullHeight + "px");
    				this.entBox.style.height = (fullHeight + "px");
    				this.objBox.style.height = (objHeight + "px");
    				
    				this._fake.globalBox.style.height = (fullHeight + "px");
    				this._fake.entBox.style.height = (fullHeight + "px");
    				this._fake.objBox.style.height = (objHeight + "px");
    				/* #Customized (김종훈) : Width, Height  오류 수정용 소스 추가 - 끝 */
            }
            ...
            //	if (this.hdr.rows.length!=2){
            		for (var i=1; i<this.hdr.rows.length; i++){
						...
						if (ha!=hb){
							/* #Customized (김종훈) : _fake Header와 Real Header가 다를 경우 작은 쪽으로 Height 고정 */
							/*  this._fake.hdr.rows[i].style.height=this.hdr.rows[i].style.height=Math.max(ha,hb)+"px"; */
							this._fake.hdr.rows[i].style.height=this.hdr.rows[i].style.height=Math.min(ha,hb)+"px";
						}
						if (window._KHTMLrv) {
							var lindex = 0;
							...
							this._fake.hdr.rows[i].childNodes[lindex].style.height=this.hdr.rows[i].childNodes[td].style.height=Math.min(ha,hb)+"px";
						}
					}
