...
function dhtmlXRibbon(struct) {
	...
	/* #Customized (조승현) : 생성자로 받은 items 보관 */
	this.item_array = [];
	if (typeof(struct) == "object" && struct.items) {
		for(i in struct.items){
			this.item_array.push(struct.items[i]);
		}
	}
...
dhtmlXRibbon.prototype._addItem = function(blockId, nextToId, itemId, data) {
	...
	if (data.type == "newLevel") {
		...
		/* #Customized(김종훈) : Ribbon 버튼 정렬 기능 추가 */
		base.className += (data.align && data.align == "right") ? " dhxrb_in_row_right" : "";
...
/* #Customized (김종훈) : 신규 Property 추가, DhtmlxRibbon Object 확인 용 */
dhtmlXRibbon.prototype.isDhtmlxRibbon = true;
