...
dhtmlXGridObject.prototype._filterA=function(column,value){
	/* #Customized (김종훈) : Calendar 관련 컬럼의 경우 - 없이 필터링, Server에서 일자 포맷이 yyyyMMdd */
	var colType = this.getColType(column);
	if(colType && (colType == "dhxCalendarA" || colType == "dhxCalendar")){
	...
	/* #Customized (김종훈) : 필터링 중 값 오류로 강제 수정 */
	for (var i=this.rowsBuffer.length-1; i>=0; i--){
		if(this._get_cell_value(this.rowsBuffer[i],column)){
			var isSplice = true;
