...
function dhtmlXGridObject(id){
	...
	/* #Customized (조승현) : div id 선언 시작*/
	this.divId = "";
	/* #Customized (조승현) : div id 선언 끝*/
	...
	if (id){
		if (typeof (id) == 'object'){
			...
			/* #Customized (조승현) : div id 저장 시작*/
			this.divId = this.entBox.parent;
			/* #Customized (조승현) : div id 저장 끝*/
		} else{
	...
	/* #Customized (조승현) 시작 : */
	this.standardColumnId = null; // 중복 검사용 컬럼 (공통함수 initGrid() 초기화 옵션)
	this.isParseDataDuplicateCheck = true; // 그리드 조회시 중복 데이터 검사 사용 여부 (공통함수 initGrid() 초기화 옵션)
	this.setDuplicationAlertMessageFunction = null; //중복 데이터 알림 메세지 세팅 함수 : *****리턴값을 메세지로 사용한다***** (공통함수 initGrid() 초기화 옵션) => 함수 정의시 파라미터로 1:중복검사열 이름, 2:중복발생값을 전달 받을 수 있음
	this.registeredIntervalIdList = []; //중복 데이터 표시용 반복 인터벌 아이디 리스트 (객체내 자동 관리 속성)
	this.isDuplicationAlertMessageShowing = false; //중복 데이터 존재 알림 표시중인지 (객체내 자동 관리 속성)
	/* #Customized (조승현) 끝 : */
	this._RaSeCol=[];
	this._wcorr=0;
	this.fontWidth = 7;
	...
	/* #Customized (김종훈) : 컬럼 맵 배열 => 컬럼 관련 Table Object */
	this.columnsMapArray={};
	...
	/* #Customized (김종훈) : Grid에 설정된 DataProcessor Object */
	this.dp=null;
	/* #Customized (김종훈) : Grid에 설정된 DataProcessor Object 반환 Function */
	this.getDataProcessor = function(){
		return this.dp;
	};
	...
	*/
	this.setSkin=function(name){
		...
		case "material":
			//this._srdh=33;
			/* #Customized Row Height 20를 기본으로, 이 수치를 기반으로 Row 생성 시 Buffer 생성에 영향 */
			this._srdh=20;			
		...
		if (el.tagName == "TD"){
			...
			if (fl&& typeof (rid) != "undefined" && cid && !skipRowSelection) {
				self.onRowSelectTime=setTimeout(function(){
						/* #Customized (김종훈) : NoDataPrintRow ID는 호출 안하게 변경 */					
						if (self.callEvent && rid !== "NoDataPrintRow")
	...
	*/
	this.clearAll=function(header){
		/* #Customized (조승현) 시작 : */
		for(var index in this.registeredIntervalIdList){
			clearInterval(this.registeredIntervalIdList[index]);
		}
		this.registeredIntervalIdList = [];
		/* #Customized (조승현) 끝 : */
	...
	*/
	...
	this.objBox.onclick=function(e){
		...
		/* #Customized (김종훈) : objBox 클릭시 편집 및 선택 Row 초기화 소스 추가 - 시작 */		
		this.firstChild.grid.editStop();
		this.firstChild.grid.clearSelection();		
		/* #Customized (김종훈) : objBox 클릭시 편집 및 선택 Row 초기화 소스 추가 - 끝 */
...
dhtmlXGridObject.prototype={
	...
	*/
	...
	/* #Customized (김종훈) : extend 방식으로 Grid Initial 하는 경우 numberFormat 세팅 호출되는 Function */
	setNumberFormatInitial:function(numberFormatArray){
		if(numberFormatArray){
			if(typeof numberFormatArray === 'object'){
				if(numberFormatArray.length > 0){
					for(var i = 0; i < numberFormatArray.length; i++){
						var numberFormatStr = numberFormatArray[i];
						if(numberFormatStr != ""){
							this.setNumberFormat(numberFormatStr, i);
						}
					}
				}
			}
	...
	*/
	changeCellType:function(r, ind, type){
		...
		var v = z.getValue();
		/* #Customized (김종훈) : 이전 Cell Type 이 popup 인 경우 클래스 제거 */
		if(z.cell && z.cell.className && z.cell.className.indexOf("dhx_popup") > -1 ){
			if(z.cell.className){
				z.cell.className = z.cell.className.replace(/ dhx_popup/gi, "");
				z.cell.className = z.cell.className.replace(/dhx_popup/gi, "");
			}
		}
	...
	*/
	...
	_postRowProcessing:function(r,donly){ 
		...
		/* #Customized(김종훈) : 조회되지 않았을 때 생성되는 Row = ID가 NoDataPrintRow 일때 onRowCreated 패스  */
		if(r.idd && r.idd == 'NoDataPrintRow'){
			return true;
		}
	...
	*/			
	parse:function(data, call, type){
		...
		/* #Customized (조승현) 시작 : */
		if(this.standardColumnId && this.isParseDataDuplicateCheck === true){
			let rowCount = this.getRowsNum();
			let cellValue = null;
			let valueObject = {};
			let duplicationColumnValueObjectInDB = {};
			let standardColumnIndex = this.getColIndexById(this.standardColumnId);
			
			for(var rowIndex = 0; rowIndex < rowCount; rowIndex++){
				cellValue = this.cells2(rowIndex, standardColumnIndex).getValue();
				if(valueObject[cellValue] != undefined && valueObject[cellValue] != null){
					valueObject[cellValue] = ++valueObject[cellValue];
					if(cellValue != undefined && cellValue != null && cellValue != ""){
						duplicationColumnValueObjectInDB[cellValue] = cellValue;
					}
				}else{
					valueObject[cellValue] = 0;
				}
			}

			let dupKeyList = [];
			let totalDupCellList = [];
			let firstDupCell = null;
			for(var dupKey in duplicationColumnValueObjectInDB){
				dupKeyList.push(dupKey);
				totalDupCellList = totalDupCellList.concat(this.findCell(dupKey, standardColumnIndex, false, true));
				if(firstDupCell == null){
					firstDupCell = totalDupCellList[0];
				}
			}
			
			if(Object.keys(duplicationColumnValueObjectInDB).length > 0){
				for(let indexC in totalDupCellList){
					let isFlag = true;
					let intervalId = setInterval(function(){
						try{
							if(isFlag){
								this.cells(totalDupCellList[indexC][0], standardColumnIndex).setBgColor('#ff3c6d');
								isFlag = false;
							}else{
								this.cells(totalDupCellList[indexC][0], standardColumnIndex).setBgColor('#ffffff');
								isFlag = true;
							}
						}catch (e) {
					...
					this.registeredIntervalIdList.push(intervalId);
				}
				
				if(firstDupCell[0] != undefined && firstDupCell[0] != null){
					this.selectRow(this.getRowIndex(firstDupCell[0]),false,false,true);
					// 1.인덱스
				...
				if(!this.isDuplicationAlertMessageShowing){
					this.isDuplicationAlertMessageShowing = true;
					
					let duplicationAlertMessage = null;
					if(this.setDuplicationAlertMessageFunction != null && typeof this.setDuplicationAlertMessageFunction == "function"){
						duplicationAlertMessage = this.setDuplicationAlertMessageFunction(this.getColLabel(standardColumnIndex), dupKeyList.join("<br>"));
					}
					
					if(duplicationAlertMessage == null){
						duplicationAlertMessage = "중복될 수 없는 열(" + this.getColLabel(standardColumnIndex) + ")에<br>중복 값이 DB내에 존재하여<br>정상적으로 작동하지 않을 수 있습니다.<br>확인 후 오류시 개발팀에 문의해주세요.<br>중복 값 : <br>" + dupKeyList.join("<br>");
					}
					
					$erp.alertMessage({
						"alertMessage" : duplicationAlertMessage,
						"alertType" : "error",
						"isAjax" : false,
						"alertCallbackFn" : function(){
							this.isDuplicationAlertMessageShowing = false;
							for(var index in this.registeredIntervalIdList){
								clearInterval(this.registeredIntervalIdList[index]);
							}
							try{
								for(var indexC in totalDupCellList){
									this.cells(totalDupCellList[indexC][0], standardColumnIndex).setBgColor('#ffffff');
								}
							}catch (e) {} // 복붙이나 업로드나 그리드 투 그리드사용시 자동으로 중복 삭제 처리되면서 셀인덱스를 벗어나는 예외가 발생할 수 있음 이미 사라진 로우이므로 무시
							this.registeredIntervalIdList = [];
						}.bind(this)
					});
				}
	...
	*/	
	addRow:function(new_id, text, ind){
		/* #Customized (김종훈) : 데이터가 존재하지 않습니다. 메시지가 출력되는 Row가 있으면 Delete */
		if(this.getRowById("NoDataPrintRow")){
			this.deleteRow("NoDataPrintRow");
		}
		/* #Customized (김종훈) : Default 값이 안오면 자동 세팅 */
		if(text == undefined || text == null){
			text = [];
			var columnCount = this.getColumnsNum();
			for(var i = 0; i < columnCount; i++){
				var colType = this.getColType(i);
				var value = "";
				if(colType == "ch" || colType == "ra" || colType == "cntr"){
					value = 0;
				}
				text.push(value);
			}
...
(function(){
		...
		var columns_map=[
			...
			/* #Customized (김종훈) : extend 방식으로 Grid Initial 하는 경우 numberFormat 세팅 Function 호출 */
			{name:"numberFormat",	def:"", 		operation:"setNumberFormatInitial",				type:direct_call}
		];
		...
		dhtmlx.extend_api("dhtmlXGridObject",{
			...
			columns:function(obj){
				/* #Customized (김종훈) : extend 방식으로 Grid Initial 하는 경우 ColumnsMapArray 보관 */
				this.columnsMapArray=obj;
...
dhtmlXGridObject.prototype._dp_init=function(dp){
	/* #Customized (김종훈) : DataProcessor Setting 시 해당 DataProcessor 저장 */
	this.dp = dp;	
...
/* #Customized (조승현) : 그리드 오브젝트, 콜백 저장 끝*/
dhtmlXGridObject.prototype.captureEventOnParentResize = function(parentLayoutObj,callback){
	parentLayoutObj.childGridObjMap[this.divId]=this;
	parentLayoutObj.childGridObjCallbackMap[this.divId]=callback;
}
/* #Customized (조승현) : 그리드 오브젝트, 콜백 저장 끝*/
...
//#}
	...
	*/
	this.setDisabled=function(fl){
		...
		if (fl != 'true'&&fl != 1) {
			if(this.cell.className && this.cell.className.indexOf("grid_cell_readonly") > -1){
...
eXcell_edn.prototype.setValue=function(val){ 
	/* #Customized (김종훈) : 수정 시 숫자가 아닌 경우 0처리 */
...
function eXcell_ron(cell){
	/*this.cell=cell;
	this.grid=this.cell.parentNode.grid;*/
	/* #Customized (김종훈) : ReadOnly 클래스 적용 - 시작 */
	if (cell){
		this.cell=cell;
		this.grid=this.cell.parentNode.grid;
	...
	/* #Customized (김종훈) : ReadOnly 클래스 적용 - 끝 */
	this.edit=function(){
	}
...
/* #Customized (조승현) : 사업자 등록 번호 용 셀 커스터마이즈 타입 생성 시작 */
function eXcell_businessNum(cell){
	if (cell){
		this.cell=cell;
		this.grid=this.cell.parentNode.grid;
		/* #Customized (김종훈) : ReadOnly 클래스 적용 */
...
//		console.log("1.showValue : " + this.cell.innerHTML.toString());
//		console.log("2.realValue : " + this.cell._orig_value);
		
		return this.cell._orig_value || $erp.getBackBusinessNum(this.cell.innerHTML.toString());
	}

	this.detach=function(){
//		console.log("detach()");
//		console.log("1.beforeValue : " + this.val);
//		console.log("2.afterValue : " + this.obj.value);
...
eXcell_businessNum.prototype=new eXcell_ed;
eXcell_businessNum.prototype.setValue=function(val){ 
...
//	console.log("1.showValue : " + $erp.getBusinessNum(val));
//	console.log("2.realValue : " + val);
	
	this.setCValue($erp.getBusinessNum(val), val);
	
}
/* #Customized (조승현) : 사업자 등록 번호 용 셀 커스터마이즈 타입 생성 끝 */
...
if (typeof(window.dhtmlXCellObject) != "undefined") {
	...
	*/
	...
	eXcell_chn.prototype=new eXcell;
	eXcell_chn.prototype.setValue=function(val){
		this.cell.style.verticalAlign="middle"; //nb:to center checkbox in line
		//val can be int
		if (val){
			val=val.toString()._dhx_trim();
		...
		var obj = this;
		this.cell.setAttribute("excell", "chn");
		this.setCValue("<img src='"+this.grid.imgURL+"item_chk"+val
