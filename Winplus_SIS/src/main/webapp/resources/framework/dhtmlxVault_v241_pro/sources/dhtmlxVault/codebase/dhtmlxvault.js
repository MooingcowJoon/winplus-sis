...
function dhtmlXVaultObject(conf) {
	...
	this.conf = {
		...
		icon_def: "",
		icons: {}, // generated
		/* #Customized(김종훈) : 임의 업로드 Parameter 선언 */
		uploadParam : null
	}
	...
	if(conf != undefined && conf != null){
		this.conf.uploadParam = conf.uploadParam;
	}
	// icons
	...
	//#Customized(김종훈)
	if(this.base.parentElement.className.indexOf("vault_essential")>-1){
		this.p_files.className = "dhx_vault_files dhx_vault_essential";
	}
...
dhtmlXVaultObject.prototype.html5.prototype = {
	...
	_doUploadFile: function(id) {
		...
		if(this.conf.uploadParam != undefined && this.conf.uploadParam != null){
			if(typeof this.conf.uploadParam === 'object' && Array === this.conf.uploadParam.constructor){
				for(var i in this.conf.uploadParam){
					var name = "param" + i;
					var value = this.conf.uploadParam[name];					
			...
			} else if (typeof this.conf.uploadParam === 'object' && Array !== this.conf.uploadParam.constructor){
				for(var i in this.conf.uploadParam){
					var name = i;
					var value = this.conf.uploadParam[name];
					if(value != undefined && value != null){
		...
		/* #Customized (김종훈) : Response Accept JSON으로 고정 */
		this.loader.setRequestHeader("Accept", "application/json, text/javascript, */*; q=0.01");		
