...
LUI = JSON.parse('${empSessionDto.lui}');
	...
	function initErpGrid(){
		erpGridColumns = [
			...
			, {id : "CUSTMR_NM", label:["공급사", "#rspan"], type: "ro", width: "150", sort : "str", align : "left", isHidden : false, isEssential : true}
	...
	function searchErpGrid(){
		...
		if($('#Set_Goods').is(":checked") == true){
			BUDL_NM = document.getElementById("setBudlNM").value;
		} else {
			BUDL_NM = "";
		}
		
		var paramData = {};
		paramDta["SEARCH_DATE_FROM"] = document.getElementById("searchDateFrom").value;
		...
		paramData["BUDL_NM"] = BUDL_NM;
		paramData["ORGN_CD"] = cmbORGN_CD.getSelectedValue();
		
		$.ajax({
			url: "/sis/market/getDoubleSalesHistory.do"
			, data : paramData
			, method : "POST"
			, dataType : "JSON"
	...
	<%-- openGoodsInformationPopup 상품정보 팝업 열림 Function --%>
		function openGoodsInformationPopup() {
			...
			console.log("goods_no 부모창 >> " + goods_no);
			console.log("inform 부모창 >> " + crud);
			$erp.openGoodsInformationPopup({"GOODS_NO" : goods_no, "CRUD" : crud});
		}
		
	<%-- openDoubleGroupPointPopup 이종상품그룹선택 팝업 열림 Function --%>
		function openDoubleGroupPointPopup(){
			var ORGN_CD = cmbORGN_CD.getSelectedValue();
			var onRowDblClicked = function(id, ind) {
				document.getElementById("Set_Goods").value = this.cells(id, this.getColIndexById("BUDL_CD")).getValue();
				document.getElementById("setBudlNM").value = this.cells(id, this.getColIndexById("BUDL_NM")).getValue();
				
				$erp.closePopup2("openDoubleGroupPointPopup");
			}
			$erp.openDoubleGroupPointPopup(onRowDblClicked, ORGN_CD);
		}
		
	<%-- dhtmlxCombo 초기화 Function 시작--%>
	function initDhtmlXCombo(){
		$('#Custmr_Search').attr("disabled", true);
		$('#setBudlNM').attr("disabled", true);
	...
	<colgroup>
		...
		<tr>
			...
			<td>
				...
				<label for="Set_Goods">이종상품그룹지정</label>
				<input type="text" id="setBudlNM" name="Set_Goods" disabled="disabled" /> 
			</td>
		</tr>
	</table>
	</div>
