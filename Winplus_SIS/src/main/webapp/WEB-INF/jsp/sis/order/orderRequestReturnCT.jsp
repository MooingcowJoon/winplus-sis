...
<script type="text/javascript">
	<%--
		※ 전역 변수 선언부( orderRequestReturnCT.jsp 센터구매반품 프로그램 )
	...
	function search_erpMiddGrid(){
		...
		$.ajax({
			...
			,data     : {
					        ...
					        , "PARAM_ORD_TYPE"    : cmv_type_cd         /* 반품유형      */
					        , "PARAM_ORD_TYPE_NM" : cmv_type_nm         /* 반품유형명    */
					        ...
					        , "PARAM_CUS_T_NM"    : "PC반품"            /* 발행구분명    */
						    , "PARAM_FLAG"        : "1"                 /* 처리상태      */
							, "PARAM_FLAG_NM"     : "반품예정"          /* 처리상태명    */
	                        , "PARAM_ROWNUM"      : 1                   /* 최저가로 가져오려면 2, else 999999 */
							, "PARAM_CUST_DIV_CD" : cmv_call_nm         /* B01물류센타로의 반품, A06신선팜으로반품, OUT외부공급사  */
	...
	function initErpRightGrid(){
		erpRightGridColumns = [
				...
				, {id : "ORD_TYPE"             , label:["요청유형"          , "#text_filter"],   type: "ro"  , width:  "60", sort : "str", align : "center", isHidden : true,  isEssential : true, commonCode : "ORD_TYPE"}			
				, {id : "ORD_TYPE_NM"          , label:["요청유형"          , "#select_filter"], type: "ro"  , width:  "80", sort : "str", align : "left",   isHidden : true , isEssential : false}			
	...
	function search_ErpRightGrid(){
		...
		$.ajax({
			...
			,data : {
				       ...
				       , "PARAM_ORD_TYPE"       : "A"                /* 1 물류,  2직납, 3신선, 7사내소비, A센터구매품 B직영점(직발)구매반품 C센터판매반품, D판매반품CS, E교환반푼CS */
					   , "PARAM_ORD_PATH"       : "1"                /* 경로 1센터, 2직영점, 3포털발주, 6긴급발주, 7사내소비 */
					   , "PARAM_RET_YN"         : "Y"                /* 반품여부  */
			}
	...
	function GoodsPricePopup_TO_ErpRightGrid(){
		...
		var onRowDblClicked = function(id) {
			...
			$erp.closePopup2("openGoodsOutMkPopup");
		}
		...
		var onClickAddData = function(erpPopupGrid) {
			...
			for(var i = 0 ; i < checkList.length ; i ++) {
				if(i != checkList.length ) {
					...
					var WARE_CD         = LOGIN_ORGN_CD;
					var WARE_NM         = LOGIN_ORGN_NM;
					...
					erpBackGrid.cells(curr_row_id, erpBackGrid.getColIndexById("ORD_CUSTMR_TYPE_NM")).setValue("PC반품");       /* 발행구분 */
					...
					erpBackGrid.cells(curr_row_id, erpBackGrid.getColIndexById("FLAG_NM"          )).setValue("반품예정");      /* 발주상태 */
		...
		//erpRightLayout.progressOn();
		openGoodsOutMkPopup(onRowDblClicked, onClickAddData);
	}
		
  	/****************************************************************************/
    /*  상품가격 팝업조회                                                       */
    /****************************************************************************/	
	openGoodsOutMkPopup = function(onRowDblClicked, onClickAddData) {

  		var onComplete = function(){
			var openPopupWindow = erpPopupWindows.window("openGoodsOutMkPopup");
			if(openPopupWindow){
		...
		var onConfirm = function(){
  		...
  		/*****************************************************************************************************/
  		var url    = "/sis/order/openGoodsOutMkPopup.sis";
		var params = {    "ORGN_DIV_CD"  :  GLOVAL_ORGN_DIV_CD
				        , "ORGN_CD"      :  GLOVAL_ORGN_CD       /* 로그인조직 */ 
				        , "CALL_CHANNEL" :  "S"                  /* S영업관리, P cs포털 */ 
				        , "CUST_DIV_CD"  :  "OUT" 
				        , "CUST_CD"      :  ""                   /* 고객사코드 */
				        , "CUST_NM"      :  ""                   /* 고객사이름 */
				        , "SUPR_CD"      :  GLOVAL_SUPR_CD       /* 협력사코드 */
				        , "SUPR_NM"      :  GLOVAL_SUPR_NM       /* 협력사이름 */
	...
	**************************************************/
	...
	function initDhtmlXCombo(){
		/* 진행상태  1:반품예정, 2:반품취소, 3:영업승인, 4:물류승인, 5:반품완료, 6:출고요청  */
	 ...
	 <div id="div_erp_contents_search" class="samyang_div" style="display:none">
	 	  <table id="aaa" class="table_search" >
			  ...
			  <tr>
				  ...
				  <th>요청일자</th>
				  <td>
					  <input type="hidden" id="txtORD_PATH"> 
