...
<script type="text/javascript">
	...
	$(document).ready(function(){		
		...
		document.getElementById("txtSUPR_CD").value  = "${param.CUST_CD}";
		document.getElementById("Custmr_Name").value = "${param.CUST_NM}";

	    if(param_CUST_CD.length == 0){
	...
	function search_erpMiddGrid(){
		...
		if ( param_CALL_CHANNEL == "P") {
			CALL_URL  = "/sis/order/getSearchMasterBarcodeCustomerLowestPriceList.do";
		}
		else
	    {   /* 팜센터에서의 신선발주 */
			if ( param_ORD_TYPE == "3") {
				CALL_URL  = "/sis/order/getSearchMasterBarcodeFreshPriceList.do";
			}
			else
			{
				CALL_URL  = "/sis/order/getSearchMasterBarcodeLowestPriceList.do";
			}
	    }
		...
		$.ajax({
			...
			,data     : {
							...
							, "PARAM_ORGN_DIV_CD"  : param_ORGN_DIV_CD   /* 부문(발주하는 조직유형)   */
							, "PARAM_ORGN_CD"      : param_ORGN_CD       /* 조직(발주하는 조직)       */
							, "PARAM_CUST_DIV_CD"  : param_CUST_DIV_CD   /* 협력사조직구분코드  B01:센터로발주, A06:팜으로발주, OUT:외부공급사로발주 */
							, "PARAM_CUST_CD"      : custmr_cd           /* 협력사코드                */
							, "PARAM_ORD_TYPE"     : param_ORD_TYPE      /* 발주유형                 */
		 	            }
			,method   : "POST"
			,dataType : "JSON"
