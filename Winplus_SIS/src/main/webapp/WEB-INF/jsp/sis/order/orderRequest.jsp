...
<script type="text/javascript">
	...
	$(document).ready(function(){		
		...
		document.getElementById("txtORD_PATH").value = "P";  /* 주문경로 P(포털), 1, 센터, 2직영점 */
		...
		$.ajax({
			...
			,success : function(data){
				...
				if(data.isError){
			        ...
			        document.getElementById("IlbanGoodsDeatline").value = data.gridDataList.CLSE_TIME;        /* 일반주문 마감시간 */
			        
			        /* 긴급주문 마감시간 */
			        document.getElementById("EmergGoodsDeatline").value = GLOVAL_ORD_SEC_TIME.substr(0,2) + ":"+ GLOVAL_ORD_SEC_TIME.substr(2,2);
			        document.getElementById("IlbaeGoodsDeatline").value = data.gridDataList.DAILY_CLSE_TIME;  /* 일배주문 마감시간 */
	...
	function isCheckedValidate(){
		...
		if(ll_cnt ==  0){
			$erp.alertMessage({
				"alertMessage" : "대상 선택을 하지않았거나 주문완료할 예정건이 없습니다.",
				"alertCode" : null,
				"alertType" : "error",
				"isAjax" : false
			});
	...
	/****************************************************************************/
    /* 대상을 가져와서 입력영역으로 옮길때 DataProcess CUD => COPY할때 전용     */
    /****************************************************************************/	
	function ObjectToEditBufferCopy(type){		
	    erpBackGrid.checkAll(true);
		...
		$erp.copyRowsGridToGrid(erpBackGrid, erpRightGrid, 
			      ...
			      [ "OBJKEY"       ,  "CHECK"           ,   "REQ_DATE"     ,   "ORD_NO"          ,   "ORD_SEQ"             ,  "ORD_TYPE"   ,
			        ...
			        "RESV_DATE"    ,  "LAND_CHG_TYPE"   ,   "MEMO"         ,   "ORD_CUSTMR_TYPE" ,   "ORD_CUSTMR_TYPE_NM"  ,  "RESP_USER"  , 
			        "EMP_NM"       ,  "FLAG"            ,   "FLAG_NM"      ,   "ORGN_DIV_CD"     ,   "ORGN_CD"             ,  "UNIT_NM" ],
	              "checked", type, [], [],{"ORD_TYPE" : cmv_type_cd, "ORD_TYPE_NM": cmv_type_nm}, {"ORD_TYPE" : cmv_type_cd, "ORD_TYPE_NM": cmv_type_nm }, callback_lastProdedure, false );
	    console.log("===========insertSelectPurordandGoods4===============");
	...
	function initErpRightGrid(){
		erpRightGridColumns = [
				...
				, {id : "ORD_TYPE_NM"          , label:["주문유형"          , "#select_filter"], type: "ro"  , width:  "80", sort : "str", align : "left",   isHidden : false , isEssential : false}			
				...
				, {id : "MIN_PUR_QTY"          , label:["물류주문수량"      , "#text_filter"],   type: "ron" , width:  "70", sort : "str", align : "right",  isHidden : false, isEssential : true,  numberFormat : "0,000.00"}			
				, {id : "MIN_PUR_UNIT"         , label:["협력사최소량"      , "#text_filter"],   type: "ron" , width:  "70", sort : "str", align : "right",  isHidden : false, isEssential : true,  numberFormat : "0,000.00"}			
	...
	function search_ErpRightGrid(){
		...
		$.ajax({
			...
			,data : {
				       ...
				       , "PARAM_RESP_USER"      : scrin_RESP_CD      /* 담당자(취급점은 표시가지 않는다) */ 
				       , "PARAM_FLAG"           : scrin_ORD_STATE
				       , "PARAM_ORD_TYPE"       : scrin_ORD_TYPE     /* 1 물류,  2직납, 3신선 */
				       , "PARAM_ORD_PATH"       : "3"                /* 경로 1센터, 2직영점, 3포털발주, 6긴급발주 */
			}
	...
	function GoodsPricePopup_TO_ErpRightGrid(){
		...
		if  (cmv_type_cd  == "1") { cmv_type_nm = "물류발주" }  
		else if  (cmv_type_cd  == "2") { cmv_type_nm = "직납발주" }
		else if  (cmv_type_cd  == "3") { cmv_type_nm = "신선발주" }
		else if  (cmv_type_cd  == "5") { cmv_type_nm = "일배발주" }
		else cmv_type_nm = "";
	...
	**************************************************/
	...
	function initDhtmlXCombo(){
		...
		/* 주문유형 */
        if ( LOGIN_ORGN_DIV_TYP == "Z") {
	   		 cmbORD_TYPE_CD = $erp.getDhtmlXComboCommonCode('cmbORD_TYPE_CD',   'cmbORD_TYPE_CD', ["ORD_TYPE","","","Z"],   177,  "모두조회", false);
        ...
        else if ( LOGIN_ORGN_DIV_TYP == "C") {
	   		 cmbORD_TYPE_CD = $erp.getDhtmlXComboCommonCode('cmbORD_TYPE_CD',   'cmbORD_TYPE_CD', ["ORD_TYPE","","MK"],   177,  "",  false, "1");
	 	  ...
	 	  <table id="aaa" class="table_search" >
			  ...
			  <tr>
				  ...
				  <td>
				      ...
				      <input type="text" id="EmergGoodsDeatline" name="EmergGoodsDeatline" readonly="readonly"  disabled="disabled" class="input_common" style="width:40px; text-align: center;" >
				  </td>
