...
<script type="text/javascript">
	...
	function initErpGrid() {
		...
		erpAllGrid.attachEvent("onRowDblClicked", function(rId){
			...
			searchDetailGrid(custmr_cd, orgn_cd);
		...
		$("#div_erp_grid").children().each(function(index, obj){
			...
			if(display == "block"){
				...
				if(grid_ID == "div_erp_all_grid"){
					...
					document.getElementById("Selected_Custmr_NM").innerHTML = "";
	...
	function SearchErpGrid(grid) {
		if(Number(($("#txtsearchDateFrom").val()).replace(/-/g,'')) > Number(($("#txtsearchDateTo").val()).replace(/-/g,''))) {
			...
			$.ajax({
				url : "/sis/report/dayByReport/getDayByCustmrList.do"
				,data : paramMap
				,method : "POST"
				,dataType : "JSON"
	...
	function searchDetailGrid(custmr_cd, orgn_cd){
		var DetailParamMap = {
		...
		erpLayout.progressOn();
		$.ajax({
			url : "/sis/report/dayByReport/getDayByCustmrDetailList.do"
			,data : DetailParamMap
			,method : "POST"
			,dataType : "JSON"
			,success : function(data) {
		...
		<div id="div_erp_contents_search" class="div_erp_contents_search" style="display:none">
			<table id="tb_erp_data" class="table_search">
				...
				<tr>
					<th>기      간</th>
					<td>
						<input type="text" id="txtsearchDateFrom" name="searchDateFrom" class="input_common input_calendar default_date" data-position="(start)">
