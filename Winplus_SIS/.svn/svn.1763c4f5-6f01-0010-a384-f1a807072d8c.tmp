...
<script type="text/javascript">
		...
		erpRibbon = new dhtmlXRibbon({
			...
			, items : [
				{type : "block", mode : 'rows', list : [
					...
					,{id : "save_erpGrid", type : "button", text:'대출입확정', isbig : false, img : "menu/apply.gif", imgdis : "menu/apply_dis.gif", disable : false}
					,{id : "delete_erpGrid", type : "button", text:'확정취소', isbig : false, img : "menu/delete.gif", imgdis : "menu/delete_dis.gif", disable : false}
				]}
			]
		});
				...
				if(gridCheckedRowArray == "NoDataPrintRow" || gridCheckedRowArray.length == 0){
					...
					for(var i=0; i<gridCheckedRowArray.length; i++){
						...
						if("Y" == erpGrid.cells(gridCheckedRowArray[i], erpGrid.getColIndexById("ORI_SEND_TYPE")).getValue()){
							isValidated = false;
							alertMessage = "전송처리가 되지 않은 대출입자료만 확정이 가능합니다.";
							break;
							...
							isValidated = false;
							alertMessage = "대출입요청 상태의 자료만 확정이 가능합니다.";
							break;
						}
					}
					...
					if(!isValidated){
						...
						$erp.confirmMessage({
							"alertMessage" : "체크된 대출입자료를 확정하시겠습니까?",
							"alertType" : "alert",
							"isAjax" : false,
							"alertCallbackFn" : function(){
								erpLayout.progressOn();
								$.ajax({
									url : "/sis/stock/conversion/confirmConvData.do" //대출입 확정
									, data : {
	...
	//대출입 확정취소
	function confirmCancelConvData(){
		var isValidated = true;
		var alertMessage = "";
		var alertType = "error";
		var alertCode = "";
			...
			if(gridCheckedRowIds == ""){
				...
				if(gridCheckedRowArray == "NoDataPrintRow" || gridCheckedRowArray.length == 0){
					...
					for(var i=0; i<gridCheckedRowArray.length; i++){
						...
						if("Y" == erpGrid.cells(gridCheckedRowArray[i], erpGrid.getColIndexById("ORI_SEND_TYPE")).getValue()){
							isValidated = false;
							alertMessage = "전송처리가 되지 않은 대출입자료만 확정취소가 가능합니다.";
							break;
						}else if("Y" == erpGrid.cells(gridCheckedRowArray[i], erpGrid.getColIndexById("REPLC_SEND_TYPE")).getValue()){
							isValidated = false;
							alertMessage = "전송처리가 되지 않은 대출입자료만 확정취소가 가능합니다.";
							break;
						}else if("3" != erpGrid.cells(gridCheckedRowArray[i], erpGrid.getColIndexById("CONV_STATE")).getValue()){
							isValidated = false;
							alertMessage = "대출입확정 상태의 자료만 확정취소가 가능합니다.";
							break;
						}
					}
					...
					if(!isValidated){
						...
						$erp.confirmMessage({
							"alertMessage" : "체크된 대출입자료를 확정취소하시겠습니까?",
							"alertType" : "alert",
							"isAjax" : false,
							"alertCallbackFn" : function(){
								erpLayout.progressOn();
								$.ajax({
									url : "/sis/stock/conversion/confirmCancelConvData.do" //대출입 확정취소
									, data : {
										"ORGN_DIV_CD" : orgnDivCd
										,"ORGN_CD" : orgnCd
										,"CONV_DATE" : convDate
	...
	function searchErpGrid(){
		...
		if(!isValidated){
			...
			erpLayout.progressOn();
			$.ajax({
				url: "/sis/stock/conversion/getStockConvReqList.do" //대출입 요청 자료 조회
				, data : {
					 "CONV_DATE" : convDate
					 , "ORGN_CD" : orgnCd
				}
	...
	function openSearchConvData(convCd){
		...
		if(convDate == null || convDate == ""){
			...
			isValidated = false;
			alertMessage = "선택된 직영점이 없습니다.";
		} else if(convCd == null || convCd == ""){
			isValidated = false;
			alertMessage = "선택된 대출입번호가 없습니다.";
		}
		...
		if(!isValidated){
			...
			var url = "/sis/stock/conversion/openSearchConvData.sis";
			var option = {
					"width" : 920
					,"height" :766
					,"resize" : false
					,"win_id" : "openSearchConvData"
			}
