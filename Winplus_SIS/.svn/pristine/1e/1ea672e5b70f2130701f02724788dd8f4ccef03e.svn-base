...
<script type="text/javascript">
		...
		for(var i = 0; i < erpGridRowCount; i++){
			...
			if(check == 1){
				...
				if  ( change_ord_statge == "2" ) {
					...
					/* 2020-01-28 WMS전송시 3번상태를 쳐답기 때문에  3발주완료 상태로 만든다 */
					Commit_flag = "3";     /* change_ord_statge; */
				}
				...
				if  ( change_ord_statge != ORD_STATE ) {
					...
					erpGridBack.cells(select_cnt, erpGridBack.getColIndexById("ORD_STATE"    )).setValue(Commit_flag);     /* 2020-01-28 change_ord_statge */
    ...
    /* erpGrid 초기화                                                           */
    /****************************************************************************/	
	function wms_call_procedure(){
    
		var BODY = $erp.dataSerializeOfGridByMode(erpGridBack,"all");   // all(전체)  state( CUD 변경부문만 처리) 
	    var url  = "/sis/order/SP_WMS_SEND_ORD_SALE.do";
		var send_data = {"BODY":BODY};    /*  LIST MAP구조로 Controller로 넘긴다 */
	...
	function initErpGrid(){
		erpGridColumns = [
				...
				, {id : "SUPR_NM"          , label:["협력사명"          , "#select_filter"], type: "ro", width: "160", sort : "str", align : "left",   isHidden : true, isEssential : true}
				...
				, {id : "REQ_DATE"         , label:["주문일자"          , "#text_filter"], type: "ro", width: "80",  sort : "str", align : "center", isHidden : false, isEssential : true}
