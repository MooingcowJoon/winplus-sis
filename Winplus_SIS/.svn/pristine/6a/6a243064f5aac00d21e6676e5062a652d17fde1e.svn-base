...
<script type="text/javascript">
	<%--
		※ 전역 변수 선언부( 주문서요청 직영점 orderRequestMk.jsp )
	...
	var GLOVAL_SUPR_NM     = "";        /* 협력사코드 : 출고창고 COMBO박스에 선택된 코드  */
	...
	var GLOVAL_WIN_PARM_CD    = "";
    var GLOVAL_WIN_PARN_NM    = "";
		...
		$.ajax({
			...
			,success : function(data){
				...
				if(data.isError){
			        ...
			        GLOVAL_WIN_PARM_CD                             = data.gridDataList.WIN_PARM_CUSTMR_CD; 			        
			        GLOVAL_WIN_PARN_NM                             = data.gridDataList.WIN_PARM_CUSTMR_NM; 	
			        ...
			        document.getElementById("IlbanGoodsDeatline").value = data.gridDataList.CLSE_TIME;        /* 일반주문마감시간 */
			        document.getElementById("IlbaeGoodsDeatline").value = data.gridDataList.DAILY_CLSE_TIME;  /* 일배주문마감시간 */
			        //document.getElementById("EmergGoodsDeatline").value = GLOVL_EME_CLSTIME.substr(0,2) + ":"+ GLOVL_EME_CLSTIME.substr(2,2);
			        document.getElementById("FreshGoodsDeatline").value = GLOVL_FRE_CLSTIME.substr(0,2) + ":"+ GLOVL_FRE_CLSTIME.substr(2,2);
					...
					console.log("GLOVAL_WIN_PARM_CD =>" + GLOVAL_WIN_PARM_CD);  
					console.log("GLOVAL_WIN_PARN_NM =>" + GLOVAL_WIN_PARN_NM);  
	...
	function searchErpLeftTree(){
		...
		if  (cmv_type_cd  == "1") { cmv_type_nm = "B01" }  
		else if  (cmv_type_cd  == "2") { cmv_type_nm = "OUT" }
		else if  (cmv_type_cd  == "3") { cmv_type_nm = "A06" }
		else if  (cmv_type_cd  == "4") { cmv_type_nm = "B01" }
		else if  (cmv_type_cd  == "5") { cmv_type_nm = "B01" }
		else cmv_type_nm = "";
		...
		$.ajax({
			...
			,data     : {   "PARAM_ORGN_DIV_CD" :  GLOVAL_ORGN_DIV_CD
				           ...
				           ,"PARAM_CUST_DIV_CD" :  cmv_type_nm   /* B01물류센타로의 주문, A06신선팜으로주문, OUT외부공급사  */ 
	...
	function initErpRightGrid(){
		erpRightGridColumns = [
				...
				, {id : "ORD_TYPE_NM"          , label:["발주유형"          , "#select_filter"], type: "ro"  , width:  "60", sort : "str", align : "center", isHidden : false, isEssential : false}			
				, {id : "FLAG_NM"              , label:["발주상태"          , "#select_filter"], type: "ro"  , width:  "60",  sort : "str", align : "center", isHidden : false , isEssential : true}
				...
				, {id : "SUPR_NM"              , label:["협력사명"          , "#select_filter"], type: "ro"  , width: "120", sort : "str", align : "left",   isHidden : false , isEssential : false}
				...
				, {id : "TAX_TYPE"             , label:["과세"              , "#text_filter"],   type: "ro"  , width:  "40", sort : "str", align : "center", isHidden : false, isEssential : false}
				, {id : "DIMEN_NM"             , label:["규격"              , "#text_filter"],   type: "ro"  , width:  "70", sort : "str", align : "left",   isHidden : false, isEssential : true}			
				, {id : "UNIT_QTY"             , label:["입수량"            , "#text_filter"],   type: "ron" , width:  "40", sort : "str", align : "right",  isHidden : false, isEssential : true,  numberFormat : "0,000.00"}			
				...
				, {id : "UNIT_NM"              , label:["단위"              , "#text_filter"],   type: "ro"  , width:  "60", sort : "str", align : "left",   isHidden : false, isEssential : true}			
				...
				, {id : "MIN_PUR_QTY"          , label:["물류주문단위"      , "#text_filter"],   type: "edn" , width:  "70", sort : "str", align : "right",  isHidden : false, isEssential : true,  numberFormat : "0,000.00"}			
				, {id : "MIN_PUR_UNIT"         , label:["주문최소량"        , "#text_filter"],   type: "edn" , width:  "70", sort : "str", align : "right",  isHidden : false, isEssential : true,  numberFormat : "0,000.00"}			
		...
		erpRightGrid.attachEvent("onEditCell", function(stage,rId,cInd,nValue,oValue) {
			...
			if ( stage == "2" &  cInd == 20 ) {
			    ...
			    var MIN_PUR_UNIT   = Number(erpRightGrid.cells(rId, erpRightGrid.getColIndexById("MIN_PUR_UNIT" )).getValue());  /* 최소주문 단위 */

			    /* 공통ORGN_CD2에  발주한도 체크여부를 체크토록 설정이 되어있으면 발주수량, 한도, 제한수량을 체크한다.*/
			    if  ( GLOVAL_LMT_CNTROL_YN == "Y") {

			    	/* 물류발주 이면 =>  최소주문 수량의 배수단위로 Validation         */
			    	if ( ls_ORD_TYPE == "1")   {
	                    ...
	                    if  ( MIN_PUR_QTY > 0 &&  ll_REQ_QTY > 0) {
					    	...
					    	if(  Number.isInteger(is_Float) == false ) {
					    		 ...
					    		 validationNotifyConfirm(rId,  1, $erp.convertToMoney(ll_MIN_PUR_QTY));
					    		 return true;
					    	}
	                    }
			    	}
			    	
			    	/* 직납발주 이면 =>   최소주문 단위값 이상으로 한도체크        */
			    	if ( ls_ORD_TYPE == "2")   {
	                    if  ( MIN_PUR_UNIT > 0 && ll_REQ_QTY > 0 ) {

	                    	if(  ll_REQ_QTY < MIN_PUR_UNIT ) {
					    		 validationNotifyConfirm(rId,  2, $erp.convertToMoney(MIN_PUR_UNIT));
					    		 return true;
					    	}
	                    }
			    	}
    ...
    /* 발주요청서에서 주문서 쪽으로 Insert                                      */
    /****************************************************************************/	
	function validationNotifyConfirm( rId, part, ord_unit ){

		var ll_cnt = 0;
		var ls_message = "";
		
		if ( part == "1") {
			ls_message = "주문 단위("+  ord_unit  + ")와 맞지 않습니다.";
		} else
		{
			ls_message = "주문 수량이 최소주문 수량 ("+  ord_unit  + ") 보다 작습니다.";
		}
		
		var callbackFunction = function(){
			
		    var ll_REQ_QTY     = ll_REQ_QTY = 0;
		    var ll_GOODS_PRICE = erpRightGrid.cells(rId, erpRightGrid.getColIndexById("GOODS_PRICE" )).getValue();
		    var ll_TAX_TYPE    = erpRightGrid.cells(rId, erpRightGrid.getColIndexById("TAX_TYPE"    )).getValue();
		    		 
		    /* 원단위 절사 */
		    var ll_SUPR_AMT  = Math.floor(ll_REQ_QTY  *  ll_GOODS_PRICE);
		    
		    var ll_VAT       = 0;
		    if  ( ll_TAX_TYPE == "Y") {
		    	ll_VAT       = Math.floor(ll_SUPR_AMT *  0.1);
		    }
	...
	function search_ErpRightGrid(){
		...
		$.ajax({
			...
			,data : {
				       ...
				       , "PARAM_SUPR_CUSTMR_CD" : scrin_SUPR_CD      /* 협력사  20191129 winplus 센터단위로 들어아해서 null처리(WP00000020) */
				       ...
				       , "PARAM_ORD_PATH"       : "2"                /* 발주를 조회하는 경로 1센터, 2직영점, 3포털발주  6긴급발주  */
			}
	...
	function GoodsPricePopup_TO_ErpRightGrid(){
		...
		var scrin_ORD_DATE = document.getElementById("txtORD_DATE").value;
		GLOVAL_SUPR_CD     = document.getElementById("hidSurp_CD").value;     /* 협력사 */
		GLOVAL_SUPR_NM     = document.getElementById("Surp_Name").value;      /* 협력사 */
		scrin_ORD_DATE     = scrin_ORD_DATE.replace (/-/g, "")
	...
	openGoodsPricePopup = function(onRowDblClicked, onClickAddData) {
		...
		var params = {    "ORGN_DIV_CD"  :  GLOVAL_ORGN_DIV_CD
				        ...
				        , "CUST_CD"      :  GLOVAL_SUPR_CD       /* 공급사코드 */
				        , "CUST_NM"      :  GLOVAL_SUPR_NM       /* 공급사이름 */
				        , "ORD_TYPE"     :  "신선발주"           /* 발주유형  1물류발주, 2직납발주, 3신선발주, 4착지변경, 5일배발주 */
		             }
	...
	**************************************************/
	...
	function initDhtmlXCombo(){
        ...
        if ( LOGIN_ORGN_DIV_TYP == "C") {
	   		 cmbORD_TYPE_CD = $erp.getDhtmlXComboCommonCode('cmbORD_TYPE_CD',   'cmbORD_TYPE_CD', ["ORD_TYPE","","MK"],   177,  "모두조회", false);
	  		   ...
	  		   cmbORD_TYPE_CD.attachEvent("onChange", function(value, text){
					...
					$erp.clearDhtmlXGrid(erpBackGrid);  //기존데이터 삭제
	  				...
	  				if  (value  == "1") { 
	  					 cmv_type_nm = "B01";
	  					 /* 협력업체를 윈플러스로 */
	 			         document.getElementById("hidSurp_CD").value    = "";  // 20191129 대표코드 아닌 센터코드로 해야해서  GLOVL_WINPLUS_CD; 
				         document.getElementById("Surp_Name").value     = "";  // GLOVL_WINPLUS_NM;
				         document.getElementById("Select_Surp_CD").checked = false;
				         Surp_Search.disabled = false;
	  				} 	else if  (value  == "2") { 
	  					 cmv_type_nm = "OUT";
	 			         document.getElementById("hidSurp_CD").value    = "";
				         document.getElementById("Surp_Name").value     = "";
				         document.getElementById("Select_Surp_CD").checked = false;
				         Surp_Search.disabled = false;
	  				} 	else if  (value  == "3") { 
	  					 cmv_type_nm = "A06";
	 			         document.getElementById("hidSurp_CD").value    = GLOVAL_WIN_PARM_CD;
				         document.getElementById("Surp_Name").value     = GLOVAL_WIN_PARN_NM;
				         document.getElementById("Select_Surp_CD").checked = true;
				         Surp_Search.disabled = true;
	  				}   else if  (value  == "4") { /* 직영점에서 착지변경은  false로 한다 */
	  					 cmv_type_nm = "B01";
	  					 /* 협력업체를 윈플러스로 */
	 			         document.getElementById("hidSurp_CD").value    = "";  // GLOVL_WINPLUS_CD;
				         document.getElementById("Surp_Name").value     = "";  // GLOVL_WINPLUS_NM;
				         document.getElementById("Select_Surp_CD").checked = false;
				         Surp_Search.disabled = false;
	  				}   else if  (value  == "5") { /* 직영점에서 일배발주  B01로 한다 */
	  					 cmv_type_nm = "B01";
	  					 /* 협력업체를 윈플러스로 */
	 			         document.getElementById("hidSurp_CD").value    = ""; // GLOVL_WINPLUS_CD;
				         document.getElementById("Surp_Name").value     = ""; // GLOVL_WINPLUS_NM;
        ...
        {
	   		 cmbORD_TYPE_CD = $erp.getDhtmlXComboCommonCode('cmbORD_TYPE_CD',   'cmbORD_TYPE_CD', 'ORD_TYPE',   177,  "모두조회",  false);
        	 $erp.objNotReadonly("cmbORD_TYPE_CD");
        }
	 	  ...
	 	  <table class="table_search" id="aaa">
			  ...
			  <tr>
				  ...
				  <td>
					  ...
					  <input type="hidden" id="txtMKT_TYPE">  <!--  주문휴형 : 1:발주, 2:주문  -->
			  ...
			  <tr>
				  ...
				  <th>마감시간(물류/일배)</th>
				  <td>
				      <input type="text" id="IlbanGoodsDeatline" name="IlbanGoodsDeatline" readonly="readonly"  disabled="disabled" class="input_common" style="width:40px; text-align: center;" >
				      <input type="text" id="IlbaeGoodsDeatline" name="IlbaeGoodsDeatline" readonly="readonly"  disabled="disabled" class="input_common" style="width:40px; text-align: center;" >
				  </td>

				  <th>마감시간(신선)</th>
				  <td><input type="text" id="FreshGoodsDeatline" name="FreshGoodsDeatline" readonly="readonly"  disabled="disabled" class="input_common" style="width:70px;"></td>
