...
function eXcell_combo(cell) {
	...
	this.edit = function(){
		...
		/* #Customized (김종훈) : disable 상태시 edit 막음, 하단에 기존 소스 주석 */
		/* 추가 및 수정 소스 시작 */
		if (!this.cell._brval) {
		...
		if(!this.combo.conf.enabled) { return false; }
		/* 추가 및 수정 소스 끝 */
		...
		/* #Customized (김종훈) : disable 상태시 edit 막기 위해 기존 소스 주석, 상단에 추가 소스 참고 */
		/*
		if (!this.cell._brval) {
			this.combo = (this.grid._realfake?this.grid._fake:this.grid)._col_combos[this.cell._cellIndex];
		} else {
	...
	this.setValue = function(val){
		...
		if (typeof(val) == "object"){
			...
			if ((cm = this.cell._brval) && (typeof(this.cell._brval) == "object")) {
				...
				val = (cm.getOption(val)||{}).text||val;
				/* #Customized (신기환) : Text로 값이 올 경우 Value를 검색하여 입력 */
				if(!cm.getOption(val) && cm.getOptionByLabel(val)) {
					this.cell.combo_value=cm.getOptionByLabel(val).value;
				}
			}
