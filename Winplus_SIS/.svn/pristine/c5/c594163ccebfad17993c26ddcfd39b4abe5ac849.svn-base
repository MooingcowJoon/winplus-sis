...
public class KisaSeedCBC {
	/* Log4j Logger */
	private static final Logger logger = LoggerFactory.getLogger(KisaSeedCBC.class);
...
//				int value = (src[b_offset/4] & mask_value) >> shift_value;
				return (byte)((src[b_offset/4] & mask_value) >> shift_value);
			} else {
				int shift_value = (b_offset%4)*8;
				int mask_value =  0x0ff << shift_value;
//				int value = (src[b_offset/4] & mask_value) >> shift_value;
...
//                (byte)0x08, (byte)0x09, (byte)0x0A, (byte)0x0B, (byte)0x0C, (byte)0x0D, (byte)0x0E, (byte)0x0F};
		...
		//logger.debug("\n");
		//logger.debug("[ Test SEED reference code CBC]"+"\n");
		//logger.debug("\n\n");
		
		
		
		//logger.debug("[ Test Encrypt mode : 방법 1 ]"+"\n");
		//logger.debug("Key\t\t\t\t: ");
	    for (int i=0; i<16; i++){
	    	//logger.debug(Integer.toHexString(0xff&pbUserKey[i])+" ");
	    }
	    //logger.debug("\n");
		//logger.debug("Plaintext\t\t\t: ");
	    for (int i=0; i<PLAINTEXT_LENGTH; i++){
	    	//logger.debug(Integer.toHexString(0xff&pbData[i])+" ");
	    }
	    //logger.debug("\n");
	    ...
	    //logger.debug("\nIV\t\t\t\t: ");
	    for (int i=0; i<16; i++){
	    	//logger.debug(Integer.toHexString(0xff&bszIV[i])+" ");
	    }
	    //logger.debug("\n");
	    
		//logger.debug("Ciphertext(SEED_CBC_Encrypt)\t: ");
	    for (int i=0; i<CIPHERTEXT_LENGTH; i++){
	    	//logger.debug(Integer.toHexString(0xff&defaultCipherText[i])+" ");
	    }
	    //logger.debug("\n");
	    
	    //logger.debug("Plaintext(SEED_CBC_Decrypt)\t: ");
	    for (int i=0; i<PLAINTEXT_LENGTH; i++){
	    	//logger.debug(Integer.toHexString(0xff&PPPPP[i])+" ");
	    }
	    //logger.debug("\n\n");
		...
		//logger.debug("Ciphertext(SEED_CBC_Encrypt1)\t: ");
	    for (int i=0; i<16; i++){
	    	//logger.debug(Integer.toHexString(0xff&Cipher1[i])+" ");
	    }
	    //logger.debug("\n");
	    
	    //logger.debug("Plaintext(SEED_CBC_Decrypt1)\t: ");
	    for (int i=0; i<2; i++){
	    	//logger.debug(Integer.toHexString(0xff&Plain1[i])+" ");
	    }
	    //logger.debug("\n\n");
		...
		//logger.debug("Ciphertext(SEED_CBC_Encrypt)\t: ");
	    for (int i=0; i<32; i++){
	    	//logger.debug(Integer.toHexString(0xff&Cipher2[i])+" ");
	    }
	    //logger.debug("\n");
	    
	    //logger.debug("Plaintext(SEED_CBC_Decrypt)\t: ");
	    for (int i=0; i<16; i++){
	    	//logger.debug(Integer.toHexString(0xff&Plain2[i])+" ");
	    }
	    //logger.debug("\n\n\n");
		...
		//logger.debug("Ciphertext(SEED_CBC_Encrypt3)\t: ");
	    for (int i=0; i<32; i++){
	    	//logger.debug(Integer.toHexString(0xff&Cipher3[i])+" ");
	    }
	    //logger.debug("\n");
	    
	    //logger.debug("Plaintext(SEED_CBC_Decrypt3)\t: ");
	    for (int i=0; i<18; i++){
	    	//logger.debug(Integer.toHexString(0xff&Plain3[i])+" ");
	    }
	    //logger.debug("\n");
		...
		//logger.debug("Ciphertext(SEED_CBC_Encrypt 1)\t: ");
	    for (int i=0; i<CIPHERTEXT_LENGTH; i++){
	    	//logger.debug(Integer.toHexString(0xff&pbszCipherText[i])+", ");
	    }
	    //logger.debug("\n");
...
//			outbuf = null;
	    ...
	    //logger.debug("Plaintext(SEED_CBC_Decrypt 1)\t: ");
	    for (int i=0; i<PLAINTEXT_LENGTH; i++){
	    	//logger.debug(Integer.toHexString(0xff&result[i])+", ");
	    }
	    //logger.debug("\n\n");
