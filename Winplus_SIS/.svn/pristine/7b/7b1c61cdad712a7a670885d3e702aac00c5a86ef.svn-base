...
function dhtmlXCalendarObject(inps, skin) {
	...
	li.onclick = function(e) {
		...
		if (t.className && t.className.indexOf("dhtmlxcalendar_month_arrow") === 0) {
			...
			/* #Customized (김종훈) : 년월 선택 기능 상태 시 전환 분기 추가 */
			if(that.conf.isMonthPicker){
				that._drawMonth(new Date(that._activeMonth.getFullYear()+ind, 0, 1, 0, 0, 0, 0));
			} else {
	...
	/* #Customized (김종훈) : 년월 선택 화면 추가 */
	this._drawMonthPicker = function(d){
		if (!(d instanceof Date)) return;
		if (isNaN(d.getFullYear())) d = new Date(this._activeMonth.getFullYear(), this._activeMonth.getMonth(), 1, 0, 0, 0, 0);
		...
		this._activeDateCell = null;
		/* var dx = new Date(this._activeDate.getFullYear(), this._activeDate.getMonth(), this._activeDate.getDate(), 0, 0, 0, 0).getTime(); */
		var dx = new Date(this._activeDate.getFullYear(), this._activeDate.getMonth(), 1, 0, 0, 0, 0).getTime();

		var mn = 0;
		for(var q=0; q<3; q++){
			for (var w=0; w<4; w++) {
				var ms = this.langData[this.lang].monthesSNames[mn];
				var time = new Date(d.getFullYear(), mn, 1, 0, 0, 0, 0).getTime();
				mn++;
				var label_css = "dhtmlxcalendar_label";
				this.contDates.childNodes[q].childNodes[w].innerHTML = "<div class='"+label_css+"'>"+ms+"</div>";
				this.contDates.childNodes[q].childNodes[w]._date = new Date(time);
				this.contDates.childNodes[q].childNodes[w]._q = q;
				this.contDates.childNodes[q].childNodes[w]._w = w;
				this.contDates.childNodes[q].childNodes[w]._css_month = true;
				this.contDates.childNodes[q].childNodes[w]._css_date = (!this._nullDate&&time==dx);
				this.contDates.childNodes[q].childNodes[w]._css_weekend = 0;
				this.contDates.childNodes[q].childNodes[w]._css_dis = this._isOutOfRange(time);
				this.contDates.childNodes[q].childNodes[w]._css_holiday = false;
				this._updateCellStyle(q, w);
...
dhtmlXCalendarObject.prototype._buildMonthHdr = function(val) {
	...
	var w = function(type, str) {
		/* #Customized (김종훈) : 년월 선택 기능 상태 시 전환 분기 추가 */
		if(that.conf && that.conf.isMonthPicker && type === 'month'){
			return "";
		}
...
/* #Customized (김종훈) : 년월 선택 전환 기능 추가 */
dhtmlXCalendarObject.prototype.setMonthPickerMode = function(){
	/* 한번 작동하면 되돌리지 못하므로 다시 인스턴스를 만들어야 함 */
	this.conf.isMonthPicker = true;
	this.contDays.firstChild.innerHTML = "";
	this.contDates.innerHTML = "";
	for (var q=0; q<3; q++) {
		var ul = document.createElement("UL");
		ul.className = "dhtmlxcalendar_line";
		this.contDates.appendChild(ul);
	...
	this._drawMonth = this._drawMonthPicker;	
	this._drawDaysOfWeek = function(){
		if (this._activeMonth != null) this._drawMonth(this._activeMonth);
	};
	this._buildMonthHdr(this._activeMonth);
	this._drawMonth(this._activeMonth);
}
