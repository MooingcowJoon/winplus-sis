...
<script type="text/javascript">
	...
	var erpPopupWindowsCell = parent.erpPopupWindows.window("openTrustSalesPopup");
	var erpLayout;
	var erpRibbon;
	var crud;
	...
	var UNIQUE_MEM_NM = "${param.UNIQUE_MEM_NM}";
	var objCD= "${param.OBJ_CD}";
	var loanCD= "${param.LOAN_CD}";
	
	$(document).ready(function(){
		if(erpPopupWindowsCell){
			erpPopupWindowsCell.setText("외상매출 결제");
		}
		...
		document.getElementById("txtPAY_DATE").value=today;
		document.getElementById("txtRESP_USER").value = loginUser;
	...
	function addErpGrid(){
		...
		var PAY_HOUR = cmbHOUR.getSelectedValue(); //시간 > 거래일자
		var PAY_MINUTE = cmbMINUTE.getSelectedValue();
		var CASH_AMT = document.getElementById("txtCASH_AMT").value; //현금
		var CARD_AMT = document.getElementById("txtCARD_AMT").value; //카드
		var ACNT_AMT = document.getElementById("txtACNT_AMT").value; //계좌이체
		
		if(!$erp.isEmpty(erpPopupGrupCheckList) && typeof erpPopupGrupCheckList === 'function'){
			erpPopupGrupCheckList(SUM_AMT, LOSS_AMT, MEMO, PAY_DATE, PAY_HOUR, PAY_MINUTE, CASH_AMT, CARD_AMT, ACNT_AMT, OBJ_CD, LOAN_CD);
		}
	}
	...
	function initDhtmlXCombo(){
		cmbHOUR = $erp.getDhtmlXComboCommonCode("cmbHOUR", "HOUR", "HOUR", 100, null, false);
		cmbMINUTE = $erp.getDhtmlXComboCommonCode("cmbHOUR", "MINUTE2", "MINUTE2", 70, null, false);
	...
	<table id="add_table01" class="table">
		...
		<tr>
			...
			<td colspan="5" >
				...
				<div id = "cmbHOUR" style="display: inline-flex; margin-right: 10px;" ></div>
				<div id = "cmbMINUTE" style="display: inline-flex;"></div>
			</td>
		</tr>
	</table>
