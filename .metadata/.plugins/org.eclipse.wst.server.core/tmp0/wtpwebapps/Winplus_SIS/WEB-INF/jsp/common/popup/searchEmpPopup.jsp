...
<script type="text/javascript">
	...
	function isSearchValidate(){
		...
		var empNm = document.getElementById("txt_EMP_NM").value;
		
		/* if($erp.isEmpty(mtrlNm) || mtrlNm.length < 2) {
			isValidated = false;
			alertMessage = "error.common.ogran.employee.name.notEnoughLength";
			alertCode = "-1";
		} else  */if(empNm.length > 50){
			isValidated = false;
	...
	function searchErpPopupGrid(){
		...
		$.ajax({
			url : "/common/popup/searchEmpPopupR1.do"
			,data : {
				"EMP_NM" : empNm
			}
			,method : "POST"
...
<div id="div_erp_contents_search" class="div_erp_contents_search" style="display:none">
	<table class="table_search">
		...
		<tr>
			<th>사용자번호/사용자명</th>
			<td>
				<input type="text" id="txt_EMP_NM" name="EMP_NM" class="input_common" maxlength="20" value="${EMP_NM}" onkeydown="$erp.onEnterKeyDown(event, searchErpPopupGrid);">
			</td>
		</tr>
	</table>
</div>
