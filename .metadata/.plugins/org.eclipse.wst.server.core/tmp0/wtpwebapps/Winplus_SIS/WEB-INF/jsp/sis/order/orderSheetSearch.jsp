...
<script type="text/javascript">
	...
	var Called_program_nm    =   "${screenDto.menu_cd}";
		...
		$.ajax({
			...
			,success : function(data){
				...
				if(data.isError){
					...
					/* 이메일 , 펙스 칼럼 숨김 처리 */
			        if 	( Called_program_nm ==  "00456" ) {    // 주문서(고객사/직영점→센터)_조회
			        	erpGrid.setColumnHidden(24,true); 
			        	erpGrid.setColumnHidden(25,true);
			        }
			        if 	( Called_program_nm ==  "00749" ) {    // 주문서(고객사/직영점→팜)_조회
			        	erpGrid.setColumnHidden(24,true); 
			        	erpGrid.setColumnHidden(25,true);
			        }
	...
	function initErpRibbon(){
        ...
        if 	( Called_program_nm ==  "00749" ) {  //  팜센터주문서조회 : 주문서(고객사/직영점→팜)_조회
			...
			erpRibbon = new dhtmlXRibbon({
				...
				, items : [
					{type : "block", mode : 'rows', list : [
							...
							, {id : "print_erpGrid",  type : "button", text:'주문서출력',                              isbig : false, img : "menu/print.gif",  imgdis : "menu/print_dis.gif",  disable : true}		
					]}	
				]
			});
        } else {   //  음성센터, 포천센터 주문서조회
			erpRibbon = new dhtmlXRibbon({
				parent : "div_erp_ribbon"
				, skin : ERP_RIBBON_CURRENT_SKINS
				, icons_path : ERP_RIBBON_CURRENT_ICON_PATH
				, items : [
					{type : "block", mode : 'rows', list : [
							  {id : "search_erpGrid", type : "button", text:'<spring:message code="ribbon.search" />', isbig : false, img : "menu/search.gif", imgdis : "menu/search_dis.gif", disable : true}
							, {id : "excel_erpGri3",  type : "button", text:'주문취소',                                isbig : false, img : "menu/add.gif",    imgdis : "menu/add_dis.gif",    disable : true}
							, {id : "excel_erpGri5",  type : "button", text:'WMS전송',                                 isbig : false, img : "menu/add.gif",    imgdis : "menu/add_dis.gif",    disable : true}
							, {id : "print_erpGrid",  type : "button", text:'주문서출력',                              isbig : false, img : "menu/print.gif",  imgdis : "menu/print_dis.gif",  disable : true}		
					]}	
				]
			});
	...
	function ProcessConfirm(change_ord_statge){
		...
		if ( change_ord_statge == "2") {
			for(var i = 0; i < erpGridRowCount; i++){
				...
				if(check == 1){
					var WMS_STATE  = erpGrid.cells(leftRid, erpGrid.getColIndexById("WMS_STATE"  )).getValue();
					if ( WMS_STATE == "03") {
		    ...
		    if ( select_cnt > 0 ) {
				$erp.alertMessage({
					"alertMessage" : "WMS 전송완료건은 취소불가능합니다.",
					"alertCode" : null,
					"alertType" : "alert",
					"isAjax" : false
				});
		...
		if ( change_ord_statge == "4") {
		    ...
		    if ( select_cnt > 0 ) {
				$erp.alertMessage({
					"alertMessage" : "WMS 전송완료건은 전송불가능합니다.",
					"alertCode" : null,
					"alertType" : "alert",
					"isAjax" : false
				});
		    ...
		    if ( select_cnt2 > 0 ) {
				$erp.alertMessage({
					"alertMessage" : "긴급발주 승인대기건은 WMS 전송불가능합니다.",
					"alertCode" : null,
					"alertType" : "alert",
					"isAjax" : false
				});
	...
	function save_mast_update(change_ord_statge){
		...
		if  (select_cnt > 0 )
		{
			...
			if  ( change_ord_statge  == "2") {  /* 주문취소 */
				url = "/sis/order/OrderStateUpdate.do";
			}  else { /* WMS전송  */
				url = "/sis/order/SP_WMS_SEND_ORD_SALE.do";
			}
	...
	function initErpGrid(){
		erpGridColumns = [
				...
				, {id : "IN_WARE_NM"       , label:["고객사"            , "#select_filter"], type: "ro", width: "160", sort : "str", align : "left",   isHidden : false, isEssential : true}
				, {id : "GOODS_NM"         , label:["주문상품"          , "#text_filter"], type: "ro", width: "250", sort : "str", align : "left",   isHidden : false, isEssential : true}			
				...
				, {id : "RESV_DATE"        , label:["납기예정일"        , "#text_filter"], type: "ro", width: "100",  sort : "str", align : "center", isHidden : false, isEssential : true}
				, {id : "ORD_TYPE"         , label:["주문유형"          , "#text_filter"], type: "ro", width: "100", sort : "str", align : "left",   isHidden : true , isEssential : true}
				, {id : "ORD_TYPE_NM"      , label:["주문유형"          , "#select_filter"], type: "ro", width: "80" , sort : "str", align : "center", isHidden : false, isEssential : true}
				, {id : "ORD_STATE"        , label:["주문상태"          , "#text_filter"], type: "ro", width: "100", sort : "str", align : "left",   isHidden : true , isEssential : true}
				, {id : "ORD_STATE_NM"     , label:["주문상태"          , "#text_filter"], type: "ro", width: "80",  sort : "str", align : "center", isHidden : false, isEssential : true}
				, {id : "WMS_STATE"        , label:["WMS상태"           , "#text_filter"], type: "ro", width: "100", sort : "str", align : "left",   isHidden : true , isEssential : true}
				, {id : "WMS_STATE_NM"     , label:["WMS상태"           , "#text_filter"], type: "ro", width: "80",  sort : "str", align : "left",   isHidden : false, isEssential : true}
	...
	function openSearchComEmpNoGridPopup() { // this는 클릭시 열리는 팝업창이다.
		var onRowSelect = function(id, ind) {
			document.getElementById("").value = this.cells(id, this.getColIndexById("EMP_NO")).getValue();
	...
	function searchErpGrid(){
        ...
        if 	( Called_program_nm ==  "00456" ) {  // 주문서(고객사/직영점→센터)_조회
        	scrn_MENU  = "CT";
        	call_URL   = "/sis/order/getOrderManagement.do"; 
        }   else if ( Called_program_nm ==  "00749" ) {  // 주문서(고객사/직영점→팜)_조회
            	scrn_MENU  = "CT";
            	call_URL   = "/sis/order/getOrderManagement.do";    /* 팜 센타로 주문한 내역만 조회 */
        } else {
        	call_URL   = "/sis/order/getSearchOrderList.do";
        	/* 발주서 조회는 센터, 직영점 orderSheetSearch.jsp 같이 사용함  */
        	if ( LOGIN_ORGN_DIV_TYP == "C") {
            	scrn_MENU  = "MK";   /* 직영점에서 센터or 외부협력사로 발주한 주문서조회 */
        	} else {
            	scrn_MENU  = "CTM";  /* 센터에서 외부협력사로 발주한 주문서조회 */
        	}
        }

        $.ajax({
			url   : call_URL /* 2020 01 04 */
			,data : {
					    ...
					    , "LOGIN_MENU"             : scrn_MENU   /*  호출메뉴  CT: 주문서센터,  MK:주문서매장 CTM:센터전용메뉴  */
					}
			,method : "POST"
			,dataType : "JSON"
			,success : function(data){
	...
	function printOrderSheet() {
		...
		var paramInfo = {
				...
				, "mrdFileName" : "order_sheet.mrd"
				, "ORGN_DIV_CD" : selected_orgn_div_cd
				, "ORGN_CD" : selected_orgn_cd
		};
		
		var approvalURL = $CROWNIX_REPORT.openOrderSheet("", paramInfo, "발주서출력", "");
