...
<script type="text/javascript">
	<%--
		※ 전역 변수 선언부( orderRequestReturnMK.jsp 직영점구매반품 프로그램 )
	...
	function isCheckedValidate(){
		...
		if(ll_cnt ==  0){
			$erp.alertMessage({
				"alertMessage" : "대상 선택하지 않았거나 반품완료할 반품예정건이 없습니다.",
				"alertCode" : null,
				"alertType" : "error",
				"isAjax" : false
			});
	...
	function GoodsPricePopup_TO_ErpRightGrid(){
		...
		if  (cmv_select_cd  == "B") {
		    if(GLOVAL_SUPR_CD.length == 0){
				$erp.alertMessage({
					"alertMessage" : "직발 반품건은 협력사를 선택하세요."
					, "alertCode"  : null
					, "alertType"  : "alert"
					, "isAjax"     : false
				});
		...
		if   (cmv_select_cd  == "B") { cmv_type_nm = "구매반품(직발)" }
		else if  (cmv_select_cd  == "C") { cmv_type_nm = "구매반품(센터)" }
		else cmv_type_nm = "";
	...
	openGoodsOutMkPopup = function(onRowDblClicked, onClickAddData) {
		...
		if   (cmv_select_cd  == "B") {
			ls_div_cd  = "OUT";  // 구매반품(직발)
			ls_supr_cd = GLOVAL_SUPR_CD;
			ls_supr_nm = GLOVAL_SUPR_NM;
		}   else {
			ls_div_cd = "B01";                        // 구매반품(센터)
			ls_supr_cd = "WP00000020";
			ls_supr_nm = "(주)윈플러스";
		}
		...
		var params = {    "ORGN_DIV_CD"  :  GLOVAL_ORGN_DIV_CD
				        ...
				        , "CUST_CD"      :  LOGIN_CUSTMR_CD      /* 고객사코드 */
				        , "CUST_NM"      :  LOGIN_CUSTMR_NM      /* 고객사이름 */
				        , "SUPR_CD"      :  ls_supr_cd           /* 협력사코드 */
				        , "SUPR_NM"      :  ls_supr_nm           /* 협력사이름 */
		              }
	...
	**************************************************/
	...
	function initDhtmlXCombo(){
        /* 직영점 반품유형  */  
		cmbORD_TYPE_CD = $erp.getDhtmlXComboCommonCode('cmbORD_TYPE_CD',   'cmbORD_TYPE_CD', ["ORD_TYPE","","","MK"],   177,  "모두조회",  false, "8");
