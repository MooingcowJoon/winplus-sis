...
<script type="text/javascript">
	<%--
		※ 전역 변수 선언부( 윈플러스팜 발주서등록  orderRequestPM.jsp )
		...
		$.ajax({
			...
			,success : function(data){
				...
				if(data.isError){
					...
					GLOVAL_MK_CNT        =   data.gridDataList.MK_CNT;          /* 직영점 매장카운터 =>  추가에서 바코드 선택시 매장수만큼 copy */
					...
					console.log("GLOVAL_MK_CNT=>"       + GLOVAL_MK_CNT);  
	...
	function searchErpLeftTree(){
		...
		//console.log("@@@@@@@@ PARAM_CUST_DIV_CD =>"+ cmv_type_nm + " OUT_CUST_CD=>" + GLOVAL_SUPR_CD   );
		...
		$.ajax({
			...
			,data     : {   "PARAM_ORGN_DIV_CD" :  GLOVAL_ORGN_DIV_CD
					       ...
					       ,"PARAM_GOODS_TYPE"  :  "3"           /* 1물류, 2직납, 3신선, 4착지 5일배  */ 
		...
		erpRightRibbon = new dhtmlXRibbon({
			...
			, items : [
				{type : "block", mode : 'rows', list : [
						...
						, {id : "excel_erpGri5",      type : "button", text:'발주요청불러오기',                        isbig : false, img : "menu/add.gif",    imgdis : "menu/add_dis.gif",    disable : true}
				]}							
			]
		});
    ...
    function CallLoadConfirm( param ){
		...
		if  (erpRightGridRowCount > 0) {
	   		...
	   		$erp.confirmMessage({
	   			  "alertMessage" : "<br>작업중인 데이타가 있습니다.<br/> 신선발주 불러오기를 하시겠습니까?"
	   			, "alertCode"    : alertCode
	   			, "alertType"    : alertType
    ...
    openOrderSecondPopup = function(onRowDblClicked, onClickAddData, param) {
        ...
        var params = {    "ORGN_DIV_CD"   : GLOVAL_ORGN_DIV_CD   /* LOGIN_ORGN_DIV_CD  */ 
                        ...
                        , "ORD_TYPE"      : param                /* 3신선발주, 4착지변경, 5일배발주 */  
    ...
    function  LoadConfirmReturnProcess(All_checkList, param) {
        ...
        scrin_ORD_DATE     = scrin_ORD_DATE.replace (/-/g, "")
   		...
   		switch (param) {
   	       case "3" :  cmv_type_nm = "신선발주"; cmv_call_nm = "OUT"; break;    /* 신선발주 */   
	...
	function insertPurordandGoodsConfirm(){
		...
		for(var i = 0; i < erpRightGridRowCount; i++){
			...
			var supr_cd = erpRightGrid.cells(rId, erpRightGrid.getColIndexById("SUPR_NM")).getValue();
	...
	function isCheckedValidate(){
		...
		if(ll_cnt ==  0){
			$erp.alertMessage({
				"alertMessage" : "발주완료할 발주예정건이 없습니다.",
				"alertCode" : null,
				"alertType" : "error",
				"isAjax" : false
			});
	...
	function ObjectToEditBuffer(type){		
		...
		$erp.copyRowsGridToGrid(erpBackGrid, erpRightGrid, 
			      [ "OBJKEY"       ,  "CHECK"           ,   "REQ_DATE"     ,   "ORD_NO"          ,   "ORD_SEQ"            ,   "ORD_TYPE"   ,
			        ...
			        "CHG_WARE_CD"  ,  "ORI_ORD_NO"      ,    "ORD_QTY"  ],
			      [ "OBJKEY"       ,  "CHECK"           ,   "REQ_DATE"     ,   "ORD_NO"          ,   "ORD_SEQ"             ,  "ORD_TYPE"   ,
			    	"ORD_TYPE_NM"  ,  "REQ_CUSTMR_CD"   ,   "CUST_NM"      ,   "SUPR_CUSTMR_CD"  ,   "SUPR_NM"             ,  "BCD_CD"     ,
			    	"BCD_NM"       ,  "WARE_CD"         ,   "WARE_NM"      ,   "TAX_TYPE"        ,   "DIMEN_NM"            ,  "UNIT_QTY"   ,
			        "STOCK_QTY"    ,  "REQ_QTY"         ,   "GOODS_PRICE"  ,   "SUPR_AMT"        ,   "VAT_AMT"             ,  "TOT_AMT"    ,
	...
	function insertSelectPurordandGoods_callback() {
		...
		console.log("===========insertSelectPurordandGoods_callback gridRowCount===============" + gridRowCount);
	...
	function initErpRightGrid(){
		erpRightGridColumns = [
				...
				, {id : "ORD_TYPE_NM"          , label:["발주유형"          , "#select_filter"], type: "ro"  , width: "70",  sort : "str", align : "left",   isHidden : true,  isEssential : false}			
				...
				/* ROW INDEX 11 ~ 20 */
				, {id : "BCD_CD"               , label:["바코드"            , "#text_filter"],   type: "ro"  , width:  "90", sort : "str", align : "left",   isHidden : false, isEssential : true}			
				...
				, {id : "CHG_WARE_CD"          , label:["배송지"            , "#text_filter"],   type: "combo", width:"100", sort : "str", align : "left",   isHidden : false , isEssential : true, commonCode : ["ORGN_CD","MK"]}
				...
				, {id : "ORD_QTY"              , label:["요청수량"          , "#text_filter"],   type: "edn" , width:  "60", sort : "str", align : "right",  isHidden : false, isEssential : true,  numberFormat : "0,000.00"}			
				, {id : "REQ_QTY"              , label:["발주수량"          , "#text_filter"],   type: "edn" , width:  "60", sort : "str", align : "right",  isHidden : false, isEssential : true,  numberFormat : "0,000.00"}			
				, {id : "CURR_QTY"             , label:["증감수량"          , "#text_filter"],   type: "edn" , width:  "60", sort : "str", align : "right",  isHidden : false, isEssential : true,  numberFormat : "0,000.00"}			
				...
				, {id : "MIN_LMT_QTY"          , label:["주문최소량"        , "#text_filter"],   type: "edn" , width:  "70", sort : "str", align : "right",  isHidden : false,  isEssential : true,  numberFormat : "0,000.00"}
	...
	function initErpBackGrid(){
		erpBackGridColumns = [
            ...
            , {id : "OBJKEY"               , label:["BCD+공급사CD"      , "#text_filter"],   type: "ro"  , width: "100", sort : "str", align : "left",   isHidden : true ,  isEssential : false}
			, {id : "MIN_LMT_QTY"          , label:["주문최소량"        , "#text_filter"],   type: "edn" , width:  "70", sort : "str", align : "right",  isHidden : false, isEssential : true,  numberFormat : "0,000.00"}			
			, {id : "CHG_WARE_CD"          , label:["배송지"            , "#text_filter"],   type: "ro"  , width: "100", sort : "str", align : "left",   isHidden : false ,  isEssential : false}
	...
	function search_ErpRightGrid(){
		...
		var scrin_ORD_TYPE  = cmbMKT_TYPE_CD.getSelectedValue();
		...
		console.log(" scrin_ORD_TYPE =>" + scrin_ORD_TYPE );

		$.ajax({
			 url  : "/sis/order/orderRequestFreshSearch.do"          /* 팜에서 외부공급사 신선발주 주문요청서 조회  */ 
			,data : {
				       ...
				       , "PARAM_MKT_TYPE"       : scrin_ORD_TYPE     /* 1.PC,  2.PDA */
				       , "PARAM_ORD_PATH"       : "1"                /* 경로 1센터, 2직영점, 3포털발주, 6긴급발주 */
			}
		...
		var onClickAddData = function(erpPopupGrid) {
			...
			for(var i = 0 ; i < checkList.length ; i ++) {
				if(i != checkList.length ) {
                    ...
                    /* 전체매장 갯수 만큼 복사를 한다 */
					for(var jj = 0 ; jj < GLOVAL_MK_CNT ; jj ++) {
						curr_row_id++;
						erpBackGrid.addRow(curr_row_id);		
						
						console.log(" curr_row_id =>" + curr_row_id  +  "  jj==>" + jj);
	...
	openGoodsPricePopup = function(onRowDblClicked, onClickAddData) {
		...
		var params = {    "ORGN_DIV_CD"  :  GLOVAL_ORGN_DIV_CD
				        ...
				        , "ORD_TYPE"     :  cmv_select_cd        /* 발주유형  1물류발주, 2직납발주, 3신선발주, 4착지변경, 5일배발주 */ 
		             }
	...
	**************************************************/
	...
	function initDhtmlXCombo(){
		...
		/* 발주유형 => 신선발주로 고정 */
  		cmbORD_TYPE_CD = $erp.getDhtmlXComboCommonCode('cmbORD_TYPE_CD',   'cmbORD_TYPE_CD', ["ORD_TYPE","PM"],   177,  "모두조회",  false, "3");
