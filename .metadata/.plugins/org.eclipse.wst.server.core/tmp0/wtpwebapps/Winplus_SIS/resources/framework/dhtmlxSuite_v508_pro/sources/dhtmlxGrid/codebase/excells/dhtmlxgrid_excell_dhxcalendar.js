...
eXcell_dhxCalendarA.prototype.detach = function() {
	...
	/* #Customized (김종훈) : 날짜 저장 시 임의 생성한 로직을 따르게 변경 */
	/* #Customized (김종훈) : 백업 시작 */
	/* this.setValue(this.obj.value); */
	/* #Customized (김종훈) : 백업 끝 */
	/* #Customized (김종훈) : 추가 소스 시작 */
	var tmpValue = $erp.getDateFormat(this.obj.value);
	if($erp.isDateValidate(tmpValue)){
		/* yyyyMMdd 20000101 양식 */
		tmpValue = tmpValue.split("-").join("");
		this.setValue(tmpValue);
	} else {
		this.setValue("");
	}
	/* #Customized (김종훈) : 추가 소스 끝 */
