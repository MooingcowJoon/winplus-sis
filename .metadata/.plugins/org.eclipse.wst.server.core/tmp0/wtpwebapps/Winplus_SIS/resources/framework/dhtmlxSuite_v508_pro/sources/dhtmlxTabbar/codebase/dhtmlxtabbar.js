...
function dhtmlXTabBar(conf, mode) { // skin? data?
	...
	/* #Customized (조승현) : 텝 선택시 리사이즈 시작 */
	this.attachEvent("onSelect", function(id, lastId){
		if(typeof (window["registered_layout"]) === "object"){
			this.resizeTabById(id);
		}
		return true;
	});
	/* #Customized (조승현) : 텝 선택시 리사이즈 끝 */
...
/* #Customized (조승현) : 선택한 텝 리사이즈 시작*/
dhtmlXTabBar.prototype.resizeTabById = function(id){
	setTimeout(function(){ /* setTiemout을 사용 이유는 즉시 호출시 텝 setSizes()가 제대로 작동하지 않는다. */
		try{
			window["registered_layout"][id].setSizes();			
		}catch (e){
			if(typeof (window["registered_layout"][id]) === "undefined" || typeof (window["registered_layout"][id]) === "null"){
				console.log("tabObj.regist(tabBarObj) 로 텝 등록이 필요합니다.");
			}
		}
	},20);
}
/* #Customized (조승현) : 선택한 텝 리사이즈 끝*/

/* #Customized (조승현) : 하위 레이아웃, 콜백 저장 시작*/
dhtmlXTabBar.prototype.captureEventOnParentResize = function(parentObj,callback){
	parentObj.childLayoutObjMap[this.divId]=this;
	parentObj.childLayoutObjCallbackMap[this.divId]=callback;
}
...
dhtmlXTabBar.prototype._setTabActive = function(id, mode) {
	...
	/* #Customized (김종훈) : Active 될 때 호출 될 Event Api 생성 */
	this.callEvent("onActive", [id]);
};
