...
/**
*  @desc: default skin for paging (Customized)
*  @author : 김종훈
*/
dhtmlXGridObject.prototype._pgn_system = function(page, start, end){
	this.totalPages=Math.ceil(this.rowsBuffer.length/this.rowsBufferOutSize);	
	if (!this.pagingBlock){
		this.pagingBlock = document.createElement("DIV");
		this.pagingBlock.className = "div_paging_block";
		if (!this._pgn_parentObj) return;
		this._pgn_parentObj.innerHTML = "";
		this._pgn_parentObj.appendChild(this.pagingBlock);
		
		var spanLeftArrow = document.createElement("SPAN");
		spanLeftArrow.className = "span_grid_paging_arrow";
		
		this.anchorLeftAbsArrow = document.createElement("A");
		this.anchorLeftAbsArrow.className = "anchor_grid_paging_arrow anchor_grid_paging_left_abs_arrow";
		this.anchorLeftAbsArrow._parent_grid = this;
		this.anchorLeftAbsArrow.onclick = function(event){
			if(event.target.getAttribute("disabled")) { return true; }
			if(event && event.target && event.target._parent_grid && event.target._parent_grid.changePage){
				event.target._parent_grid.changePage(1);
				event.target._parent_grid.systemCurrentPage.value=event.target._parent_grid.currentPage;
				/* #Customized (조승현) : grid 페이징 버튼 완료시점 추가 시작*/
				this._parent_grid.callEvent("onPageChangeCompleted",[this._parent_grid.getStateOfView()]);
				/* #Customized (조승현) : grid 페이징 버튼 완료시점 추가 종료*/
			}
		};
		this.anchorLeftArrow = document.createElement("A");
		this.anchorLeftArrow.className = "anchor_grid_paging_arrow anchor_grid_paging_left_arrow";
		this.anchorLeftArrow._parent_grid = this;
		this.anchorLeftArrow.onclick = function(event){
			if(event.target.getAttribute("disabled")) { return true; }
			if(event && event.target && event.target._parent_grid && event.target._parent_grid.changePageRelative){
				event.target._parent_grid.changePageRelative(-1);
				event.target._parent_grid.systemCurrentPage.value=event.target._parent_grid.currentPage;
				/* #Customized (조승현) : grid 페이징 버튼 완료시점 추가 시작*/
				this._parent_grid.callEvent("onPageChangeCompleted",[this._parent_grid.getStateOfView()]);
				/* #Customized (조승현) : grid 페이징 버튼 완료시점 추가 종료*/
		...
		spanLeftArrow.appendChild(this.anchorLeftAbsArrow);
		spanLeftArrow.appendChild(this.anchorLeftArrow);
		this.pagingBlock.appendChild(spanLeftArrow);
		
		this.systemCurrentPage = document.createElement("INPUT");
		this.systemCurrentPage._parent_grid = this;
		this.systemCurrentPage.className = "input_paging";
		this.systemCurrentPage.setAttribute("type", "text");
		this.systemCurrentPage.onblur = function(event){
			...
			if(event.target.value > event.target._parent_grid.totalPages){
				event.target.value = event.target._parent_grid.totalPages;
			}
			if(event.target._parent_grid.currentPage != event.target.value){
				if(event.target._parent_grid.changePage && typeof event.target._parent_grid.changePage === 'function'){
					event.target._parent_grid.changePage(event.target.value);
		   		}
			}
		};
		this.systemCurrentPage.onkeydown = function(event){
			var keyCode = (window.event) ? event.which : event.keyCode;
		   	if (keyCode == 13) {
		   		if(event.target && event.target.blur){
		   			event.target && event.target.blur(event);
		   		} else if(event.target && event.target.onblur){
		   			event.target && event.target.onblur(event);
		   		};
		      return false;
		   	}
		   	return true;
		}
		this.systemCurrentPage.value = page;
		this.pagingBlock.appendChild(this.systemCurrentPage);
		
		var spanSeparate = document.createElement("SPAN");
		spanSeparate.className = "span_grid_paging_separate";
		spanSeparate.innerText = " / ";
		this.pagingBlock.appendChild(spanSeparate);
		
		this.systemTotalPage = document.createElement("INPUT");
		this.systemTotalPage.className = "input_paging input_readonly";
		this.systemTotalPage.setAttribute("readonly", "readonly");
		
		this.pagingBlock.appendChild(this.systemTotalPage);
		
		var spanRightArrow = document.createElement("SPAN");
		spanRightArrow.className = "span_grid_paging_arrow";
		
		this.anchorRightArrow = document.createElement("A");
		this.anchorRightArrow.className = "anchor_grid_paging_arrow anchor_grid_paging_right_arrow";
		this.anchorRightArrow._parent_grid = this;
		this.anchorRightArrow.onclick = function(event){
			if(event.target.getAttribute("disabled")) { return true; }
			if(event && event.target && event.target._parent_grid && event.target._parent_grid.changePageRelative){
				event.target._parent_grid.changePageRelative(1);
				event.target._parent_grid.systemCurrentPage.value=event.target._parent_grid.currentPage;
		...
		this.anchorRightAbsArrow = document.createElement("A");
		this.anchorRightAbsArrow.className = "anchor_grid_paging_arrow anchor_grid_paging_right_abs_arrow";
		this.anchorRightAbsArrow._parent_grid = this;
		this.anchorRightAbsArrow.onclick = function(event){
			if(event.target.getAttribute("disabled")) { return true; }
			if(event && event.target && event.target._parent_grid && event.target._parent_grid.changePage){
				event.target._parent_grid.changePage(event.target._parent_grid.totalPages);
				event.target._parent_grid.systemCurrentPage.value=event.target._parent_grid.currentPage;
				/* #Customized (조승현) : grid 페이징 버튼 완료시점 추가 시작*/
				this._parent_grid.callEvent("onPageChangeCompleted",[this._parent_grid.getStateOfView()]);
				/* #Customized (조승현) : grid 페이징 버튼 완료시점 추가 종료*/
		...
		spanRightArrow.appendChild(this.anchorRightArrow);
		spanRightArrow.appendChild(this.anchorRightAbsArrow);
		
		this.pagingBlock.appendChild(spanRightArrow);
	}
	if(this.systemCurrentPage){
		this.systemCurrentPage.value = this.currentPage;
	}
	if(this.systemTotalPage){
		this.systemTotalPage.value = this.totalPages;
	}
	if(this.currentPage == 1){
		this.anchorLeftArrow.setAttribute("disabled", "disabled");
		this.anchorLeftAbsArrow.setAttribute("disabled", "disabled");
	} else {
		this.anchorLeftArrow.removeAttribute("disabled");
		this.anchorLeftAbsArrow.removeAttribute("disabled");
	}
	if(this.currentPage == this.totalPages){
		this.anchorRightArrow.setAttribute("disabled", "disabled");
		this.anchorRightAbsArrow.setAttribute("disabled", "disabled");
	} else {
		this.anchorRightArrow.removeAttribute("disabled");
		this.anchorRightAbsArrow.removeAttribute("disabled");
	}
