...
function dhtmlXCombo(parentId, formName, width, optionType, tabIndex) {
	...
	/* #Customized (조승현) 시작 : */
	this.isMultiCheckReadonly = false; //멀티체크 콤보박스 사용시 수정은 못하고 펼쳐서 볼수만 있도록 하는 용도
	this.onCheck_whenMultiCheckReadonly = null; //멀티체크 리드온리 사용시 자동 등로하는 이벤트 번호
	/* #Customized (조승현) 끝 : */
	...
	this.base.innerHTML = "<input type='text' class='dhxcombo_input' style='width:"+(this.conf.combo_width-(this.conf.i_ofs+1)-(this.conf.combo_image?this.conf.i_ofs:0))+"px;"+"' autocomplete='off'>"+
...
function dhtmlXComboFromSelect(selectId, width) {	
	...
	/* #Customized (김종훈) : param에 온 Width 있을 경우 해당 Width 사용 */
...
dhtmlXCombo.prototype._getOption = function(id, index) {
	...
	/* #Customized (조승현) : 아이템 옵션값 추가 커스터마이징 시작*/
	var custom_t = this.t[id].item._conf;
	t = $erp.unionObjArray([t,custom_t]);
	/* #Customized (조승현) : 아이템 옵션값 추가 커스터마이징 끝*/
...
/* #Customized (조승현) 시작 : */
dhtmlXCombo.prototype.multiCheckReadonly = function(isReadonly){ //멀티체크박스 리드온리 사용여부
	if(isReadonly != undefined && isReadonly != null){
		var whenUserCheckTryMessage = "수정 할 수 없습니다."; //멀티 체크박스가 리드온리상태인데 사용자가 수정하려고 할때 보여줄 메세지
		if(typeof isReadonly == "string" && isReadonly != ""){
			this.isMultiCheckReadonly = true;
			whenUserCheckTryMessage = isReadonly;
		}else if(isReadonly === true){
			this.isMultiCheckReadonly = true;
		}else{
			this.isMultiCheckReadonly = false;
		}
		
		if(this.isMultiCheckReadonly){
			if(this.onCheck_whenMultiCheckReadonly == null){
				this.onCheck_whenMultiCheckReadonly = this.attachEvent("onCheck", function(value, checkState, isProgramTry){ //값, 체크상태, 프로그램으로 강제 호출 한 것인지
					if(isProgramTry == undefined || isProgramTry == null || isProgramTry != true){
						this.setChecked(this.getOption(value).index, !checkState);
						$erp.alertMessage({
							"alertMessage" : whenUserCheckTryMessage,
							"alertType" : "alert",
							"isAjax" : false
						});
					}
			...
			if(this.onCheck_whenMultiCheckReadonly != null){
				this.detachEvent(this.onCheck_whenMultiCheckReadonly);
				this.onCheck_whenMultiCheckReadonly = null;
			}
		}
	}
}
